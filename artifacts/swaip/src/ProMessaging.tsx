...
export function ChatScreen({ convId, otherHash, otherInfo, myHash, accent, onBack, isGroup: isGroupProp, groupName: groupNameProp, participants: participantsProp, isSecret: isSecretProp, onViewProfile }: {
  ...
  const [forwardingMsg,    setForwardingMsg]    = useState<MsgItem|null>(null);
  const [showSearch,       setShowSearch]       = useState(false);
  const [searchQuery,      setSearchQuery]      = useState('');
  /* ── Запланированные сообщения (реальный API) ── */
  /* ── Исчезающие сообщения (обычный чат) ── */
  const [regularBurnMs,    setRegularBurnMs]    = useState(0);
  const [showRegularBurn,  setShowRegularBurn]  = useState(false);
  const [showSchedule,     setShowSchedule]     = useState(false);
  const [scheduleAt,       setScheduleAt]       = useState('');
  const [scheduledMsgs,    setScheduledMsgs]    = useState<{id:string;text:string;sendAt:number}[]>([]);
  /* ── Видеокружочки ── */
  const [circleOpen,       setCircleOpen]       = useState(false);
  const [circleRecording,  setCircleRecording]  = useState(false);
  const [circleSec,        setCircleSec]        = useState(0);
  const schedLoadedRef = useRef(false);
  const gameSession = useGameSession(convId ?? null, myHash);

  /* ── Load scheduled messages from API on mount ── */
  useEffect(() => {
    if (!convId || schedLoadedRef.current) return;
    schedLoadedRef.current = true;
    fetch(`${API_BASE}/api/scheduled-messages?conversationId=${convId}`, { headers: { 'x-session-token': getST() } })
      .then(r => r.ok ? r.json() : null)
      .then((d: any) => {
        if (d?.scheduledMessages) {
          setScheduledMsgs(d.scheduledMessages.map((m: any) => ({ id: m.id, text: m.content, sendAt: m.sendAt })));
        }
      }).catch(() => {});
  }, [convId]);
  ...
  const circleMediaRef   = useRef<MediaRecorder | null>(null);
  const circleChunksRef  = useRef<Blob[]>([]);
  const circleTimerRef   = useRef<ReturnType<typeof setInterval> | null>(null);
  const circleSecRef     = useRef(0);
  ...
  /* ── Timer countdown for burnAt messages (all types) ── */
  useEffect(() => {
    const seed: Record<number,string> = {};
    messages.forEach(m => { if (m.burnAt) seed[m.id] = '...'; });
    if (Object.keys(seed).length) setMsgTimers(prev => ({ ...prev, ...seed }));
  }, [messages.length]);

  useEffect(() => {
    const t = setInterval(() => {
      const burnMsgs = messages.filter(m => m.burnAt);
      if (!burnMsgs.length) return;
      ...
      setMsgTimers(prev => {
        ...
        for (const msg of burnMsgs) {
          ...
          if (diff <= 0) { expired.push(msg.id); changed = true; continue; }
      ...
      if (expired.length) {
        setMessages(prev => prev.filter(m => !expired.includes(m.id)));
      }
    }, 1000);
    return () => clearInterval(t);
  }, [messages]);
  ...
  const sendMsg = async (content: string, type = 'text', mediaUrl?: string, mediaName?: string, dur?: number, rplId?: number, burnInMs?: number) => {
    if (sending) return;
    ...
    try {
      ...
      if (burnInMs && burnInMs > 0) body.burnInMs = burnInMs;
      const r = await fetch(`${API_BASE}/api/conversations/${convId}/messages`, {
        method:'POST', headers:{ 'Content-Type':'application/json', 'x-session-token': getST() },
  ...
  const handleSend = async () => {
    ...
    setInput(''); await sendMsg(txt, 'text', undefined, undefined, undefined, replyTarget?.id, regularBurnMs > 0 ? regularBurnMs * 1000 : undefined);
  };
  ...
  /* ══ ВИДЕОКРУЖОЧКИ ══ */
  const openCircleCamera = async () => {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode:'user', width:{ ideal:640 }, height:{ ideal:640 }, aspectRatio:1 },
        audio: true,
      });
  ...
  const closeCircleCamera = () => {
    circleStreamRef.current?.getTracks().forEach(t => t.stop());
    circleStreamRef.current = null;
    if (circleTimerRef.current) { clearInterval(circleTimerRef.current); circleTimerRef.current = null; }
    if (circleMediaRef.current) { try { circleMediaRef.current.stop(); } catch {} circleMediaRef.current = null; }
    circleChunksRef.current = [];
    setCircleOpen(false); setCircleRecording(false); setCircleSec(0); circleSecRef.current = 0;
  };

  const startCircleRec = () => {
    if (!circleStreamRef.current) return;
    const mime = ['video/mp4;codecs=avc1,mp4a.40.2','video/webm;codecs=vp9,opus','video/webm;codecs=vp8,opus','video/webm','video/mp4'].find(t => MediaRecorder.isTypeSupported(t)) ?? '';
    const opts = mime ? { mimeType:mime, videoBitsPerSecond:900_000, audioBitsPerSecond:64_000 } : {};
    const mr = new MediaRecorder(circleStreamRef.current, opts);
    circleChunksRef.current = [];
    mr.ondataavailable = e => { if (e.data.size > 0) circleChunksRef.current.push(e.data); };
    mr.start(200); circleMediaRef.current = mr;
    circleSecRef.current = 0;
    setCircleRecording(true); setCircleSec(0);
    circleTimerRef.current = setInterval(() => {
      circleSecRef.current += 1;
      setCircleSec(circleSecRef.current);
      if (circleSecRef.current >= 60) stopCircleRec();
    }, 1000);
  };

  const stopCircleRec = async () => {
    if (!circleMediaRef.current) return;
    const mr = circleMediaRef.current; const dur = circleSecRef.current;
    circleMediaRef.current = null;
    if (circleTimerRef.current) { clearInterval(circleTimerRef.current); circleTimerRef.current = null; }
    setCircleRecording(false); setCircleSec(0); setCircleOpen(false); circleSecRef.current = 0;
    await new Promise<void>(resolve => { mr.onstop = () => resolve(); mr.stop(); });
    await new Promise(r => setTimeout(r, 300));
    circleStreamRef.current?.getTracks().forEach(t => t.stop()); circleStreamRef.current = null;
    const chunks = [...circleChunksRef.current]; circleChunksRef.current = [];
    if (chunks.length === 0) return;
    const mimeType = mr.mimeType || 'video/webm';
    const ext = mimeType.includes('mp4') ? 'mp4' : 'webm';
    const blob = new Blob(chunks, { type:mimeType });
    const file = new File([blob], `circle_${Date.now()}.${ext}`, { type:mimeType });
    setUploadingFile('кружочек…');
    try {
      const url = await uploadToStorage(file);
      const body = { content:'videoCircle', messageType:'videoCircle', mediaUrl:url, mediaName:file.name, duration:dur };
      const r = await fetch(`${API_BASE}/api/conversations/${convId}/messages`, {
        method:'POST', headers:{ 'Content-Type':'application/json', 'x-session-token':getST() },
        body: JSON.stringify(body),
      });
      if (r.ok) { const msg = await r.json(); setMessages(p => [...p, msg]); }
    } catch { alert('Не удалось отправить кружочек.'); }
    finally { setUploadingFile(null); }
  };
    ...
    <div style={{ position:'fixed', inset:0, background:C.bg, display:'flex', flexDirection:'column', zIndex:300 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px 10px 36px',
        ...
        <motion.button whileTap={{ scale:0.88 }}
          ...
          style={{ background: showSearch ? `${eff}33` : 'rgba(255,255,255,0.07)',
            border: `1px solid ${showSearch ? eff : 'transparent'}`, borderRadius:'50%',
            width:36, height:36, cursor:'pointer', fontSize:15,
            display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0,
            color: showSearch ? eff : 'rgba(255,255,255,0.7)' }} title="Поиск в чате">🔍</motion.button>
      </div>
      ...
      <AnimatePresence>
        {showSearch && (
          <motion.div initial={{ height:0, opacity:0 }} animate={{ height:'auto', opacity:1 }} exit={{ height:0, opacity:0 }}
            ...
            <div style={{ padding:'8px 12px', background:'rgba(8,10,18,0.98)',
              borderBottom:`1px solid ${eff}22` }}>
              <div style={{ display:'flex', alignItems:'center', gap:8, background:'rgba(255,255,255,0.07)',
                borderRadius:12, padding:'8px 12px', border:`1px solid ${eff}33` }}>
              ...
              {searchQuery.trim() && (
                <div style={{ fontSize:10, color:'rgba(255,255,255,0.3)', fontFamily:'"Montserrat",sans-serif',
                  marginTop:5, paddingLeft:4 }}>
                  Найдено: {messages.filter(m => m.content.toLowerCase().includes(searchQuery.toLowerCase())).length} сообщений
                </div>
              )}
            </div>
          </motion.div>
              ...
              {isSystem ? (
                ...
                <div
                  ...
                  <div style={{ maxWidth:'72%', display:'flex', flexDirection:'column', alignItems:isMe?'flex-end':'flex-start' }}>
                    ...
                    {m.burnAt && (
                      <div style={{ fontSize:9, fontWeight:800, color: m.messageType==='secret_swipe' ? '#a855f7' : '#ef4444', marginBottom:2, fontFamily:'"Montserrat",sans-serif',
                        display:'flex', alignItems:'center', gap:3 }}>
                        <span>{m.messageType==='secret_swipe' ? '🕵️' : '🔥'}</span>
                        ...
                        <>
                          ...
                          {m.messageType==='videoMessage' && m.mediaUrl && (
                            <div style={{ display:'flex', flexDirection:'column', alignItems: isMe ? 'flex-end' : 'flex-start', gap:4 }}>
                              <div style={{ position:'relative', width:168, height:168, borderRadius:'50%', overflow:'hidden',
                                boxShadow:`0 4px 20px rgba(0,0,0,0.6), 0 0 0 3px ${eff}55`, flexShrink:0 }}>
                                <video src={m.mediaUrl} controls playsInline
                                  style={{ width:'100%', height:'100%', display:'block', objectFit:'cover' }} />
                                {/* Gradient ring */}
                                <div style={{ position:'absolute', inset:0, borderRadius:'50%',
                                  boxShadow:`inset 0 0 0 3px ${eff}88`, pointerEvents:'none' }}/>
                              </div>
                              {m.duration && (
                                <div style={{ fontSize:10, color:'rgba(255,255,255,0.5)', fontFamily:'"Montserrat",sans-serif',
                                  display:'flex', alignItems:'center', gap:4 }}>
                                  <span>🎥</span>
                                  <span>{String(Math.floor(m.duration/60)).padStart(2,'0')}:{String(m.duration%60).padStart(2,'0')}</span>
                                </div>
                              )}
                            </div>
                          )}
                          {m.messageType==='videoCircle' && m.mediaUrl && (
                            <div style={{ display:'flex', flexDirection:'column', alignItems: isMe ? 'flex-end' : 'flex-start', gap:4 }}>
                              <div style={{ position:'relative', width:148, height:148, borderRadius:'50%', overflow:'hidden', flexShrink:0,
                                boxShadow:`0 4px 24px rgba(0,0,0,0.7), 0 0 0 3px ${eff}88`, background:'#000' }}>
                                <video src={m.mediaUrl} controls playsInline
                                  style={{ width:'100%', height:'100%', display:'block', objectFit:'cover' }} />
                                <div style={{ position:'absolute', inset:0, borderRadius:'50%',
                                  boxShadow:`inset 0 0 0 3px ${eff}`, pointerEvents:'none' }}/>
                                <div style={{ position:'absolute', top:6, left:6, background:'rgba(0,0,0,0.55)',
                                  borderRadius:20, padding:'2px 7px', fontSize:9, color:'#fff', fontWeight:800,
                                  fontFamily:'"Montserrat",sans-serif', display:'flex', alignItems:'center', gap:3 }}>
                                  <span>🔵</span><span>Кружочек</span>
                                </div>
                              </div>
                              {m.duration && (
                                <div style={{ fontSize:10, color:'rgba(255,255,255,0.45)', fontFamily:'"Montserrat",sans-serif',
                                  display:'flex', alignItems:'center', gap:4 }}>
                                  <span>🔵</span>
                                  <span>{String(Math.floor(m.duration/60)).padStart(2,'0')}:{String(m.duration%60).padStart(2,'0')}</span>
                                </div>
      ...
      {/* ══ Видеокружочек — оверлей записи ══ */}
      <AnimatePresence>
        {circleOpen && (
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
            style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.96)', zIndex:210,
              display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
            {/* Закрыть */}
            <motion.button whileTap={{ scale:0.88 }} onClick={closeCircleCamera}
              style={{ position:'absolute', top:52, left:20, background:'rgba(255,255,255,0.1)',
                border:'none', borderRadius:'50%', width:44, height:44, cursor:'pointer',
                fontSize:18, display:'flex', alignItems:'center', justifyContent:'center',
                color:'#fff', zIndex:10 }}>✕</motion.button>
            {/* Заголовок */}
            <div style={{ position:'absolute', top:62, left:0, right:0, textAlign:'center',
              fontSize:13, fontWeight:700, color:'rgba(255,255,255,0.7)',
              fontFamily:'"Montserrat",sans-serif' }}>
              {circleRecording
                ? `⏺ ${String(Math.floor(circleSec/60)).padStart(2,'0')}:${String(circleSec%60).padStart(2,'0')} / 01:00`
                : '🔵 Видеокружочек'}
            </div>
            {/* Круглый превью + прогресс-кольцо */}
            <div style={{ position:'relative', width:264, height:264 }}>
              {circleRecording && (
                <svg style={{ position:'absolute', top:-4, left:-4, width:272, height:272, zIndex:3 }} viewBox="0 0 272 272">
                  <circle cx="136" cy="136" r="132" fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="5"/>
                  <circle cx="136" cy="136" r="132" fill="none" stroke={eff} strokeWidth="5"
                    strokeDasharray={`${2*Math.PI*132}`}
                    strokeDashoffset={`${2*Math.PI*132*(1 - circleSec/60)}`}
                    strokeLinecap="round" transform="rotate(-90 136 136)"
                    style={{ transition:'stroke-dashoffset 1s linear' }}/>
                </svg>
              )}
              <div style={{ width:264, height:264, borderRadius:'50%', overflow:'hidden',
                border: circleRecording ? `4px solid ${eff}` : '4px solid rgba(255,255,255,0.18)',
                boxShadow: circleRecording ? `0 0 40px ${eff}55` : '0 8px 40px rgba(0,0,0,0.7)' }}>
                <video
                  ref={el => {
                    circlePreviewRef.current = el;
                    if (el && circleStreamRef.current) {
                      el.srcObject = circleStreamRef.current;
                      el.play().catch(() => {});
                    }
                  }}
            ...
            <div style={{ marginTop:36, display:'flex', flexDirection:'column', alignItems:'center', gap:12 }}>
              <motion.button
                whileTap={{ scale:0.88 }}
                onClick={circleRecording ? stopCircleRec : startCircleRec}
                animate={circleRecording ? { scale:[1,1.06,1] } : {}}
                transition={{ repeat: circleRecording ? Infinity : 0, duration:0.9 }}
                style={{ width:72, height:72, borderRadius:'50%', border:'none', cursor:'pointer',
                  background: circleRecording ? '#ef4444' : eff,
                  boxShadow: circleRecording
                    ? '0 0 28px rgba(239,68,68,0.65)'
                    : `0 0 28px ${eff}66`,
                  display:'flex', alignItems:'center', justifyContent:'center', fontSize:30 }}>
                {circleRecording ? '⏹' : '⏺'}
              </motion.button>
              <div style={{ fontSize:12, color:'rgba(255,255,255,0.45)', fontFamily:'"Montserrat",sans-serif', fontWeight:600 }}>
                {circleRecording ? 'Нажми чтобы остановить' : 'Нажми чтобы начать запись'}
              </div>
            </div>
          </motion.div>
        )}
        ...
        {contextMenu && (
          <div style={{ position:'fixed', inset:0, zIndex:500 }}
            ...
            <motion.div initial={{ scale:0.85, opacity:0 }} animate={{ scale:1, opacity:1 }} exit={{ scale:0.85, opacity:0 }}
              ...
              {/* Переслать */}
              <button onClick={() => { setForwardingMsg(contextMenu.msg); setShowForwardModal(true); setContextMenu(null); }}
                style={{ width:'100%', background:'none', border:'none', padding:'10px 16px', color:'#fff', fontSize:14,
                  fontFamily:'"Montserrat",sans-serif', cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:10 }}>
                📤 Переслать
              </button>
              {/* Копировать текст */}
              {contextMenu.msg.messageType === 'text' && !contextMenu.msg.deletedForAll && (
                <button onClick={() => {
                  try { navigator.clipboard.writeText(contextMenu.msg.content); } catch {}
                  setContextMenu(null);
                }}
                  style={{ width:'100%', background:'none', border:'none', padding:'10px 16px', color:'#fff', fontSize:14,
                    fontFamily:'"Montserrat",sans-serif', cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:10 }}>
      ...
      {recording ? (
        ...
        <div style={{ position:'relative' }}>
          ...
          <AnimatePresence>
            {showMoreMenu && !isSecret && (
              <motion.div
                initial={{ height:0, opacity:0 }} animate={{ height:'auto', opacity:1 }}
                exit={{ height:0, opacity:0 }} transition={{ duration:0.18 }}
                style={{ overflow:'hidden', background:'rgba(10,12,20,0.97)', borderTop:'1px solid rgba(255,255,255,0.07)' }}>
                <div style={{ padding:'14px 12px 10px', display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8 }}>
                  {/* File */}
                  <label style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:5, cursor:'pointer',
                    background:'rgba(255,255,255,0.05)', borderRadius:14, padding:'12px 4px' }}>
                    <span style={{ fontSize:24 }}>📎</span>
                    <span style={{ fontSize:10, color:'rgba(255,255,255,0.5)', fontWeight:700, fontFamily:'"Montserrat",sans-serif' }}>Файл</span>
                    <input type="file" style={{ display:'none' }} multiple accept="image/*,video/*,audio/*,.pdf,.doc,.docx,.gif"
                      onChange={e => { Array.from(e.target.files||[]).forEach(f=>handleFile(f)); e.target.value=''; setShowMoreMenu(false); }} />
                  </label>
                  {/* Video */}
                  <motion.button whileTap={{ scale:0.9 }} onClick={() => { openVideoCamera(); setShowMoreMenu(false); }}
                    style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:5, cursor:'pointer',
                      background:'rgba(239,68,68,0.08)', borderRadius:14, padding:'12px 4px', border:'none' }}>
                    <span style={{ fontSize:24 }}>🎥</span>
                    <span style={{ fontSize:10, color:'rgba(255,255,255,0.5)', fontWeight:700, fontFamily:'"Montserrat",sans-serif' }}>Видео</span>
                  </motion.button>
                  {/* Circle */}
                  <motion.button whileTap={{ scale:0.9 }} onClick={() => { openCircleCamera(); setShowMoreMenu(false); }}
                    style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:5, cursor:'pointer',
                      background:'rgba(99,102,241,0.08)', borderRadius:14, padding:'12px 4px', border:'none' }}>
                    <span style={{ fontSize:24 }}>🔵</span>
                    <span style={{ fontSize:10, color:'rgba(255,255,255,0.5)', fontWeight:700, fontFamily:'"Montserrat",sans-serif' }}>Кружочек</span>
                  </motion.button>
                  {/* Voice hold */}
                  <motion.button whileTap={{ scale:0.9 }}
                    ...
                    style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:5, cursor:'pointer',
                      background:'rgba(59,130,246,0.08)', borderRadius:14, padding:'12px 4px', border:'none' }}>
                    <span style={{ fontSize:24 }}>🎙</span>
                    <span style={{ fontSize:10, color:'rgba(255,255,255,0.5)', fontWeight:700, fontFamily:'"Montserrat",sans-serif' }}>Голос</span>
                  </motion.button>
                  {/* BG Music */}
                  <motion.button whileTap={{ scale:0.9 }} onClick={() => { setShowBgMusicPicker(true); setShowMoreMenu(false); }}
                    style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:5, cursor:'pointer',
                      background:'rgba(236,72,153,0.08)', borderRadius:14, padding:'12px 4px', border:'none' }}>
                    <span style={{ fontSize:24 }}>🎵</span>
                    <span style={{ fontSize:10, color:'rgba(255,255,255,0.5)', fontWeight:700, fontFamily:'"Montserrat",sans-serif' }}>Музыка</span>
                  </motion.button>
                  {/* Invite link */}
                  <motion.button whileTap={{ scale:0.9 }} onClick={() => {
                    ...
                    background:'rgba(201,152,58,0.08)', borderRadius:14, padding:'12px 4px', border:'none' }}>
                    <span style={{ fontSize:24 }}>🔗</span>
                    <span style={{ fontSize:10, color:'rgba(255,255,255,0.5)', fontWeight:700, fontFamily:'"Montserrat",sans-serif' }}>Круг</span>
                  </motion.button>
                  {/* Burn timer */}
                  <div style={{ position:'relative' }}>
                    <motion.button whileTap={{ scale:0.9 }} onClick={() => setShowRegularBurn(s=>!s)}
                      style={{ width:'100%', display:'flex', flexDirection:'column', alignItems:'center', gap:5, cursor:'pointer',
                        background: regularBurnMs > 0 ? 'rgba(239,68,68,0.18)' : 'rgba(239,68,68,0.06)',
                        borderRadius:14, padding:'12px 4px', border:'none' }}>
                      <span style={{ fontSize:24 }}>{regularBurnMs > 0 ? '🔥' : '⏱'}</span>
                      <span style={{ fontSize:10, color: regularBurnMs > 0 ? '#ef4444' : 'rgba(255,255,255,0.5)', fontWeight:700, fontFamily:'"Montserrat",sans-serif' }}>
                        {regularBurnMs > 0 ? BURN_OPTIONS.find(o=>o.value===regularBurnMs)?.label : 'Удалять'}
                      </span>
                    </motion.button>
                    {showRegularBurn && (
                      <div style={{ position:'absolute', bottom:'110%', left:'50%', transform:'translateX(-50%)',
                        background:'#0f1823', border:'1px solid rgba(239,68,68,0.4)', borderRadius:14, overflow:'hidden',
                        zIndex:200, boxShadow:'0 8px 32px rgba(0,0,0,0.8)', minWidth:140, whiteSpace:'nowrap' }}>
                        <div style={{ fontSize:9, fontWeight:900, color:'rgba(255,255,255,0.35)',
                          padding:'8px 12px 4px', letterSpacing:2, textTransform:'uppercase',
                          borderBottom:'1px solid rgba(255,255,255,0.07)' }}>УДАЛЯТЬ ЧЕРЕЗ</div>
                        {BURN_OPTIONS.map(opt => (
                          <div key={opt.value} onClick={e => { e.stopPropagation(); setRegularBurnMs(opt.value); setShowRegularBurn(false); }}
                            style={{ padding:'9px 14px', cursor:'pointer', fontSize:13, fontFamily:'"Montserrat",sans-serif',
                              color: regularBurnMs === opt.value ? '#ef4444' : '#fff',
                              background: regularBurnMs === opt.value ? 'rgba(239,68,68,0.1)' : 'transparent',
                              fontWeight: regularBurnMs === opt.value ? 800 : 400,
                              display:'flex', alignItems:'center', gap:8 }}>
                            <span>{regularBurnMs === opt.value ? '✓' : ' '}</span>{opt.label}
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  {/* Schedule */}
                  <div style={{ position:'relative' }}>
                    <motion.button whileTap={{ scale:0.9 }} onClick={() => setShowSchedule(s=>!s)}
                      style={{ width:'100%', display:'flex', flexDirection:'column', alignItems:'center', gap:5, cursor:'pointer',
                        background: showSchedule ? 'rgba(139,92,246,0.18)' : 'rgba(139,92,246,0.06)',
                        borderRadius:14, padding:'12px 4px', border:'none' }}>
                      <span style={{ fontSize:24 }}>⏰</span>
                      <span style={{ fontSize:10, color:'rgba(255,255,255,0.5)', fontWeight:700, fontFamily:'"Montserrat",sans-serif' }}>Расписание</span>
                    </motion.button>
                    {showSchedule && (
                      <div style={{ position:'absolute', bottom:'110%', right:0, background:'#0f1823',
                        border:'1px solid rgba(139,92,246,0.4)', borderRadius:14, padding:14,
                        zIndex:200, boxShadow:'0 8px 32px rgba(0,0,0,0.8)', minWidth:260, whiteSpace:'nowrap' }}
                        onClick={e => e.stopPropagation()}>
                        <div style={{ fontSize:11, fontWeight:900, color:'rgba(255,255,255,0.4)',
                          letterSpacing:2, textTransform:'uppercase', marginBottom:10 }}>ЗАПЛАНИРОВАТЬ ОТПРАВКУ</div>
                        <input type="datetime-local"
                          min={new Date(Date.now()+60000).toISOString().slice(0,16)}
                          value={scheduleAt} onChange={e => setScheduleAt(e.target.value)}
                          style={{ width:'100%', padding:'8px 10px', borderRadius:8, border:'1px solid rgba(139,92,246,0.4)',
                            background:'rgba(139,92,246,0.08)', color:'#fff', fontSize:13, outline:'none',
                            fontFamily:'"Montserrat",sans-serif', boxSizing:'border-box' }} />
                        <motion.button whileTap={{ scale:0.95 }} disabled={!scheduleAt || !input.trim()}
                          onClick={async () => {
                            if (!scheduleAt || !input.trim()) return;
                            const sendAt = new Date(scheduleAt).getTime();
                            if (sendAt <= Date.now()) return;
                            const text = input.trim();
                            setInput(''); setScheduleAt(''); setShowSchedule(false); setShowMoreMenu(false);
                            try {
                              const r = await fetch(`${API_BASE}/api/scheduled-messages`, {
                                method:'POST', headers:{ 'Content-Type':'application/json', 'x-session-token': getST() },
                                body: JSON.stringify({ conversationId:convId, content:text, sendAt:new Date(scheduleAt).toISOString() }),
                              });
                              if (r.ok) { const d = await r.json(); if (d.scheduledMessage) setScheduledMsgs(p => [...p, { id:d.scheduledMessage.id, text, sendAt }]); }
                            } catch {}
                          }}
                          style={{ marginTop:10, width:'100%', padding:'9px 0', borderRadius:10,
                            background: scheduleAt && input.trim() ? 'rgba(139,92,246,0.7)' : 'rgba(139,92,246,0.2)',
                            border:'none', color:'#fff', fontWeight:800, fontSize:13, cursor:'pointer',
                            fontFamily:'"Montserrat",sans-serif', opacity:(!scheduleAt||!input.trim())?0.5:1 }}>
                          ✓ Запланировать
                        </motion.button>
                        {scheduledMsgs.length > 0 && (
                          <div style={{ marginTop:10, borderTop:'1px solid rgba(255,255,255,0.08)', paddingTop:8 }}>
                            <div style={{ fontSize:10, color:'rgba(255,255,255,0.3)', fontWeight:700, marginBottom:6 }}>ЗАПЛАНИРОВАНО ({scheduledMsgs.length})</div>
                            {scheduledMsgs.slice(-3).map(s => (
                              <div key={s.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, marginBottom:4 }}>
                                <div style={{ fontSize:11, color:'rgba(255,255,255,0.6)', flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                                  {new Date(s.sendAt).toLocaleString('ru',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}: {s.text.slice(0,30)}{s.text.length>30?'…':''}
                                </div>
                                <button onClick={async () => {
                                  setScheduledMsgs(p => p.filter(m => m.id !== s.id));
                                  try { await fetch(`${API_BASE}/api/scheduled-messages/${s.id}`, { method:'DELETE', headers:{ 'x-session-token': getST() } }); } catch {}
                                }} style={{ background:'none', border:'none', color:'rgba(239,68,68,0.6)', cursor:'pointer', fontSize:12, padding:'0 2px' }}>✕</button>
                              </div>
                            ))}
                          </div>
                        )}
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 10px',
            ...
            {/* + More */}
            {!isSecret && (
              <motion.button whileTap={{ scale:0.88 }} onClick={() => setShowMoreMenu(s=>!s)}
                style={{ background: showMoreMenu ? `${eff}22` : 'rgba(255,255,255,0.07)',
                  border: showMoreMenu ? `1.5px solid ${eff}66` : '1px solid rgba(255,255,255,0.12)',
                  borderRadius:'50%', width:36, height:36, cursor:'pointer', fontSize:20,
                  display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0,
                  color: showMoreMenu ? eff : 'rgba(255,255,255,0.7)', fontWeight:900,
...
/* ══════════════════════════════════════════════
   FORWARD MODAL
══════════════════════════════════════════════ */
function ForwardModal({ msg, accent, onClose }: { msg: MsgItem; accent: string; onClose: () => void }) {
  const [convs,   setConvs]   = useState<ConvItem[]>([]);
  const [sending, setSending] = useState<number | null>(null);
  const [done,    setDone]    = useState<number[]>([]);

  useEffect(() => {
    fetch(`${API_BASE}/api/conversations`, { headers: { 'x-session-token': getST() } })
      .then(r => r.json()).then(d => setConvs(d.conversations ?? [])).catch(() => {});
  }, []);

  const forward = async (convId: number) => {
    setSending(convId);
    try {
      const r = await fetch(`${API_BASE}/api/conversations/${convId}/messages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-session-token': getST() },
        body: JSON.stringify({ content: `📤 Переслано:\n${msg.content}`, messageType: 'text' }),
      });
      if (r.ok) setDone(p => [...p, convId]);
    } catch {} finally { setSending(null); }
  };

  return (
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
      style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.85)', zIndex:600,
        display:'flex', alignItems:'flex-end', backdropFilter:'blur(6px)' }}
      onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <motion.div initial={{ y:'100%' }} animate={{ y:0 }} exit={{ y:'100%' }}
        transition={{ type:'spring', damping:30, stiffness:300 }}
        onClick={e => e.stopPropagation()}
        style={{ width:'100%', background:'#0e0e16', borderRadius:'24px 24px 0 0',
          padding:'18px 16px calc(24px + env(safe-area-inset-bottom,0px))',
          maxHeight:'78vh', overflowY:'auto', boxShadow:'0 -8px 40px rgba(0,0,0,0.6)',
          border:'1px solid rgba(255,255,255,0.08)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
          <span style={{ fontSize:20 }}>📤</span>
          <p style={{ margin:0, fontSize:16, fontWeight:900, color:'#fff', flex:1, fontFamily:'"Montserrat",sans-serif' }}>
            Переслать сообщение
          </p>
        ...
        <div style={{ background:'rgba(255,255,255,0.05)', borderRadius:12, padding:'10px 14px',
          marginBottom:14, fontSize:13, color:'rgba(255,255,255,0.55)', fontFamily:'"Montserrat",sans-serif',
          borderLeft:`3px solid ${accent}`, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
          {msg.content.slice(0, 100) || '📎 Медиафайл'}
        </div>
        {convs.length === 0 && (
          <div style={{ textAlign:'center', padding:'24px 0', color:'rgba(255,255,255,0.3)',
            fontFamily:'"Montserrat",sans-serif' }}>
            <div style={{ fontSize:36, marginBottom:8 }}>💬</div>Нет чатов
          </div>
        )}
        {convs.map(cv => {
          const isGrp  = cv.type === 'group' || cv.type === 'broadcast';
          const name   = isGrp ? (cv.name || 'Беседа') : cv.otherInfo.name;
          const isDone = done.includes(cv.id);
          return (
            <motion.div key={cv.id} whileTap={{ scale:0.98 }}
              style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 0',
                borderBottom:'1px solid rgba(255,255,255,0.05)', cursor: isDone ? 'default' : 'pointer' }}
              onClick={() => !isDone && !sending && forward(cv.id)}>
              <div style={{ width:46, height:46, borderRadius:'50%', flexShrink:0,
                background:`linear-gradient(135deg,${accent}44,${accent}22)`,
                border:`2px solid ${accent}44`,
                display:'flex', alignItems:'center', justifyContent:'center', fontSize:20 }}>
                {isGrp ? '👥' : (cv.otherInfo.name?.[0]?.toUpperCase() || '?')}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:14, fontWeight:800, color:'#fff', fontFamily:'"Montserrat",sans-serif',
                  overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{name}</div>
                {cv.otherInfo.handle && (
                  <div style={{ fontSize:11, color:'rgba(255,255,255,0.35)', fontFamily:'"Montserrat",sans-serif' }}>
                    @{cv.otherInfo.handle}
                  </div>
                )}
              </div>
              <div style={{ fontSize:12, fontWeight:800, fontFamily:'"Montserrat",sans-serif', flexShrink:0,
                color: isDone ? '#22c55e' : sending===cv.id ? 'rgba(255,255,255,0.4)' : accent }}>
                {sending===cv.id ? '…' : isDone ? '✓ Отправлено' : 'Переслать'}
              </div>
            </motion.div>
          );
        })}
          ...
          <motion.div whileTap={{ scale:0.98 }}
            onClick={async () => {
              try {
                const r = await fetch(`${API_BASE}/api/conversations`, {
                  method:'POST', headers:{ 'Content-Type':'application/json', 'x-session-token': getST() },
                  body: JSON.stringify({ otherHash: myHash }),
                });
                if (r.ok) {
                  const { conversation: cv } = await r.json();
                  setActiveConv({ id:cv.id, otherHash:myHash,
                    otherInfo:{ name:'Избранное', avatar:'', handle:'' },
                    isGroup:false, isBroadcast:false, isSecret:false, groupName:null, participants:[] });
                }
              } catch {}
            }}
            ...
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontWeight:800, fontSize:15, color:'#fff', fontFamily:'"Montserrat",sans-serif' }}>
                Избранное
              </div>
              <div style={{ fontSize:12, color:'rgba(255,255,255,0.38)', fontFamily:'"Montserrat",sans-serif' }}>
