...
export const SERVICE_CATEGORIES = [
  { id:'beauty',     emoji:'💇', label:'Красота',          keywords:['парикмахер','стрижка','маникюр','педикюр','барбер','барбершоп','бровист','визажист','стилист','косметолог','спа','салон','ногти','окрашивание','укладка','прическа','шугаринг','эпиляция','наращивание'] },
  { id:'repair',     emoji:'🔧', label:'Ремонт',           keywords:['ремонт','сантехник','электрик','отделка','строитель','сварщик','плиточник','кровельщик','штукатур','монтаж','установка','мастер','починить','сантехника','электрика','трубы','проводка','плитка','отделочник'] },
  { id:'cleaning',   emoji:'🧹', label:'Уборка',           keywords:['уборка','уборщица','клининг','чистка','генеральная уборка','мойка','химчистка'] },
  { id:'education',  emoji:'📚', label:'Обучение',         keywords:['репетитор','учитель','курсы','обучение','преподаватель','уроки','тренинг','коуч','английский','математика','язык','школа','образование','тьютор'] },
  { id:'medicine',   emoji:'🏥', label:'Здоровье',         keywords:['врач','доктор','медицина','здоровье','стоматолог','психолог','терапевт','массаж','массажист','лечение','клиника','медицинский','хирург','педиатр','гинеколог','аллерголог','мануальный'] },
  { id:'auto',       emoji:'🚗', label:'Авто',             keywords:['авто','машина','автосервис','шиномонтаж','мойка авто','автомойка','детейлинг','автозапчасти','ремонт авто','автоэлектрик','кузовной','полировка','тонировка','диагностика'] },
  { id:'delivery',   emoji:'📦', label:'Доставка',         keywords:['доставка','курьер','грузоперевозки','переезд','грузчик','такси','логистика','транспорт','перевозка','газель'] },
  { id:'food',       emoji:'🍳', label:'Еда и кулинария',  keywords:['кондитер','торт','выпечка','еда','питание','кейтеринг','повар','шеф','кулинария','суши','пицца','домашняя еда','бургер','пирог','доставка еды'] },
  { id:'photo',      emoji:'📸', label:'Фото и видео',     keywords:['фотограф','фотосессия','видеограф','видеосъёмка','монтаж','оператор','съёмка','видео','фото','репортаж','свадебный фотограф','детская фотосессия'] },
  { id:'events',     emoji:'🎉', label:'Праздники',        keywords:['тамада','ведущий','аниматор','праздник','организатор мероприятий','декоратор','свадьба','день рождения','корпоратив','шары','фокусник','клоун','детский праздник'] },
  { id:'fitness',    emoji:'💪', label:'Спорт и фитнес',   keywords:['тренер','фитнес','спорт','йога','бокс','танцы','пилатес','тренажёрный зал','личный тренер','бассейн','зумба','растяжка','кроссфит','карате','самбо','айкидо'] },
  { id:'pets',       emoji:'🐾', label:'Животные',         keywords:['ветеринар','груминг','зооуслуги','передержка','дрессировка','питомник','кошки','собаки','зоогостиница','ветклиника','вет','животные'] },
  { id:'legal',      emoji:'⚖️', label:'Юридические',      keywords:['юрист','адвокат','нотариус','бухгалтер','налоги','юридические услуги','договор','иск','консультация юриста','регистрация','бухгалтерия'] },
  { id:'realty',     emoji:'🏠', label:'Недвижимость',     keywords:['аренда','квартира','недвижимость','риелтор','купить квартиру','сдам','снять','продам','дом','жильё','агент','ипотека','посуточно','комната'] },
  { id:'it',         emoji:'💻', label:'IT и технологии',  keywords:['программист','разработчик','сайт','веб','it','айти','технологии','приложение','смм','seo','дизайнер','верстальщик','1с','системный администратор','настройка компьютера'] },
  { id:'creative',   emoji:'🎨', label:'Творчество',       keywords:['художник','иллюстратор','дизайн','искусство','музыкант','певец','актёр','творчество','рисование','живопись','скульптор','крафт','hand made','рукоделие'] },
  { id:'nanny',      emoji:'👶', label:'Дети и семья',     keywords:['няня','нянечка','сиделка','воспитатель','детский сад','уход за детьми','ребёнок','дети','семья','babysitter','присмотр'] },
  { id:'business',   emoji:'💼', label:'Бизнес и продажи', keywords:['бизнес','продажи','маркетинг','реклама','торговля','оптовая торговля','магазин','интернет-магазин','предприниматель','стартап','партнёрство','инвестиции'] },
  { id:'sport',      emoji:'⚽', label:'Спорт',            keywords:['спорт','футбол','волейбол','баскетбол','теннис','хоккей','спортивный клуб','секция','тренировки'] },
  { id:'travel',     emoji:'✈️', label:'Путешествия',      keywords:['туризм','путешествие','тур','экскурсия','гид','отдых','турагент','виза','отель','путёвка'] },
  { id:'music',      emoji:'🎵', label:'Музыка',           keywords:['музыкант','вокалист','певец','музыка','студия','запись','аранжировка','урок вокала','урок гитары','музыкальная школа','dj','диджей','продюсер'] },
  { id:'other',      emoji:'🌟', label:'Другое',           keywords:[] },
];

export function detectCategory(tags: string[], name: string, description: string): string {
  const haystack = [...tags, name, description].join(' ').toLowerCase();
  let best = 'other', bestScore = 0;
  for (const cat of SERVICE_CATEGORIES) {
    if (cat.id === 'other') continue;
    const score = cat.keywords.filter(k => haystack.includes(k)).length;
    if (score > bestScore) { bestScore = score; best = cat.id; }
  }
  return best;
}

const CHANNEL_CATEGORIES = SERVICE_CATEGORIES;
...
/* ══════════════════════════════════════════════════════
   ХРАНИЛИЩЕ
══════════════════════════════════════════════════════ */
const _chTok=()=>{try{return localStorage.getItem('swaip_session')||'';}catch{return'';}};

function _chSummary(chs:SwaipChannel[]){
  return chs.map(ch=>({
...
function useChannelsStore(userHash:string):[SwaipChannel[],React.Dispatch<React.SetStateAction<SwaipChannel[]>>] {
  ...
  const [channels,setChannelsRaw]=useState<SwaipChannel[]>(()=>{
    try{
      ...
      if(parsed.length){
        try{localStorage.setItem(SYNC_KEY,JSON.stringify(_chSummary(parsed)));}catch{}
      }
      return parsed;
    }catch{return[];}
  });

  /* ── Load from API on mount (overwrites localStorage if API has fresher data) ── */
  useEffect(()=>{
    if(!userHash)return;
    const tok=_chTok();
    if(!tok)return;
    fetch(`${window.location.origin}/api/channels-sync`,{headers:{'x-session-token':tok}})
      .then(r=>r.ok?r.json():null)
      .then(d=>{
        if(!d||!Array.isArray(d.channels))return;
        const apiChannels:SwaipChannel[]=d.channels;
        /* Prefer API data; merge: if local has channels API doesn't, keep them */
        setChannelsRaw(local=>{
          const apiIds=new Set(apiChannels.map(c=>c.id));
          /* Channels in local but not in API (created offline) */
          const onlyLocal=local.filter(c=>!apiIds.has(c.id));
          const merged=[...apiChannels,...onlyLocal];
          if(merged.length===local.length&&JSON.stringify(merged.map(c=>c.id))===JSON.stringify(local.map(c=>c.id))&&
             apiChannels.length===0&&local.length>0){
            /* API empty but local has data — push local to API */
            const t=_chTok();
            if(t)fetch(`${window.location.origin}/api/channels-sync`,{method:'PUT',headers:{'Content-Type':'application/json','x-session-token':t},body:JSON.stringify({channels:local})}).catch(()=>{});
            return local;
          }
          try{localStorage.setItem(KEY,JSON.stringify(merged));}catch{}
          try{localStorage.setItem(SYNC_KEY,JSON.stringify(_chSummary(merged)));}catch{}
          return merged;
        });
      })
      .catch(()=>{});
  ...
  const setChannels:React.Dispatch<React.SetStateAction<SwaipChannel[]>>=(action)=>{
    setChannelsRaw(prev=>{
      ...
      try{
        ...
        localStorage.setItem(SYNC_KEY,JSON.stringify(_chSummary(next)));
        try{window.dispatchEvent(new CustomEvent('sw:channels-updated'));}catch{}
      }catch{}
      /* Persist to API in background */
      const tok=_chTok();
      if(tok){
        fetch(`${window.location.origin}/api/channels-sync`,{
          method:'PUT',
...
/* ══════════════════════════════════════════════════════
   ХРАНИЛИЩЕ ГРУПП
══════════════════════════════════════════════════════ */
function _grSummary(gs:SwaipGroup[]){
  return gs.map(g=>({
...
function useGroupsStore(userHash:string):[SwaipGroup[],React.Dispatch<React.SetStateAction<SwaipGroup[]>>] {
  ...
  const [groups,setGroupsRaw]=useState<SwaipGroup[]>(()=>{
    try{
      ...
      if(parsed.length){
        try{localStorage.setItem(SYNC_KEY,JSON.stringify(_grSummary(parsed)));}catch{}
      }
      return parsed;
    }catch{return[];}
  });
  ...
  useEffect(()=>{
    ...
    const tok=_chTok();
    if(!tok)return;
    fetch(`${window.location.origin}/api/groups-sync`,{headers:{'x-session-token':tok}})
      .then(r=>r.ok?r.json():null)
      .then(d=>{
        if(!d||!Array.isArray(d.groups))return;
        const apiGroups:SwaipGroup[]=d.groups;
        setGroupsRaw(local=>{
          const apiIds=new Set(apiGroups.map(g=>g.id));
          const onlyLocal=local.filter(g=>!apiIds.has(g.id));
          const merged=[...apiGroups,...onlyLocal];
          if(apiGroups.length===0&&local.length>0){
            /* Push local to API */
            const t=_chTok();
            if(t)fetch(`${window.location.origin}/api/groups-sync`,{method:'PUT',headers:{'Content-Type':'application/json','x-session-token':t},body:JSON.stringify({groups:local})}).catch(()=>{});
            return local;
          }
          try{localStorage.setItem(KEY,JSON.stringify(merged));}catch{}
          try{localStorage.setItem(SYNC_KEY,JSON.stringify(_grSummary(merged)));}catch{}
          return merged;
        });
      })
      .catch(()=>{});
  ...
  const setGroups:React.Dispatch<React.SetStateAction<SwaipGroup[]>>=(action)=>{
    setGroupsRaw(prev=>{
      ...
      try{
        localStorage.setItem(KEY,JSON.stringify(next));
        localStorage.setItem(SYNC_KEY,JSON.stringify(_grSummary(next)));
        try{window.dispatchEvent(new CustomEvent('sw:groups-updated'));}catch{}
      }catch{}
      /* Persist to API in background */
      const tok=_chTok();
      if(tok){
        fetch(`${window.location.origin}/api/groups-sync`,{
          method:'PUT',
          headers:{'Content-Type':'application/json','x-session-token':tok},
...
export default function ChannelsScreen({ userHash, isDark, c, accent, userName, userAvatar, isActive, apiBase='' }: Props) {
  ...
  const bookPost=(chId:string,postId:string,time?:string,clientName?:string,clientPhone?:string)=>{
    setChannels(cs=>cs.map(ch=>{
      if(ch.id!==chId)return ch;
      const post=ch.posts.find(p=>p.id===postId);
      if(clientName&&clientPhone&&post){
        fetch(`${window.location.origin}/api/booking-request`,{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({targetHash:userHash,clientName,clientPhone,service:post.bookingLabel||'Запись',slot:time||'',
            text:`Запись: ${post.bookingLabel||'Запись'}. Слот: ${time||''}`})}).catch(()=>{});
      }
      return{...ch,posts:ch.posts.map(p=>{
          ...
          !openId?(
            ...
            openCh&&(
              <ChannelPage ch={openCh} c={c} accent={accent} isDark={isDark}
                ...
                userHash={userHash}
                onBook={(postId,time,cName,cPhone)=>bookPost(openCh.id,postId,time,cName,cPhone)}
                onEditBooking={(postId,patch)=>editPostBooking(openCh.id,postId,patch)}
                activeRubric={activeRubric} onRubric={setActiveRubric}
                onCompose={()=>setShowCompose(true)}
                onAddPost={addPost} tick={tick} userName={userName}/>
            )
          )
        )}
        {activeTab==='groups'&&(
  ...
  return (
    <motion.div whileTap={{scale:0.98}} onClick={onOpen}
      ...
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        <span style={{fontSize:11,color:c.sub}}>👁 {fmtNum(post.views)}</span>
...
══════════════════════════════════════════════════════ */
function ChannelInvitePanel({ch,c,accent,onUpdate,userName}:{
...
/* ══════════════════════════════════════════════════════
   СТРАНИЦА КАНАЛА
══════════════════════════════════════════════════════ */
function ChannelPage({ch,c,accent,isDark,onBack,onReact,onVote,onOpenCapsule,onPin,onDelete,onUpdate,onBook,onEditBooking,activeRubric,onRubric,onCompose,onAddPost,tick,userName,userHash}:{
  ...
  onUpdate:(patch:Partial<SwaipChannel>)=>void;
  onBook:(postId:string,time?:string,clientName?:string,clientPhone?:string)=>void;
  ...
  /* ── CRM заказов канала ── */
  const [showChannelCRM,setShowChannelCRM]=useState(false);
  const [channelOrders,setChannelOrders]=useState<any[]>([]);
  const [channelOrdersLoading,setChannelOrdersLoading]=useState(false);
  const [newOrderCount,setNewOrderCount]=useState(0);

  const fetchChannelOrders=()=>{
    const tok=_getTok();if(!tok)return;
    setChannelOrdersLoading(true);
    fetch(`${window.location.origin}/api/account/booking-requests`,{headers:{'x-session-token':tok}})
      .then(r=>r.ok?r.json():null)
      .then(d=>{
        if(d?.requests){
          const all:any[]=d.requests;
          const forChannel=all.filter((r:any)=>r.channelId===ch.id||(r.channelName&&r.channelName===ch.name)||r.text?.includes(`Канал: ${ch.name}`)||r.text?.includes(ch.name));
          setChannelOrders(forChannel);
          const newCount=forChannel.filter((r:any)=>r.status==='new').length;
          setNewOrderCount(newCount);
        }
      }).catch(()=>{}).finally(()=>setChannelOrdersLoading(false));
  };

  useEffect(()=>{
    fetchChannelOrders();
    const iv=setInterval(fetchChannelOrders,20000);
    return()=>clearInterval(iv);
  },[ch.id]);

  useEffect(()=>{
    const handler=()=>fetchChannelOrders();
    window.addEventListener('sw-new-booking',handler);
    return()=>window.removeEventListener('sw-new-booking',handler);
  },[]);

  const updateChannelOrderStatus=async(id:string,status:string)=>{
    const tok=_getTok();if(!tok)return;
    await fetch(`${window.location.origin}/api/account/booking-requests/${id}`,{
      method:'PATCH',headers:{'Content-Type':'application/json','x-session-token':tok},
      body:JSON.stringify({status})}).catch(()=>{});
    setChannelOrders(prev=>prev.map(r=>r.id===id?{...r,status}:r));
    if(status!=='new')setNewOrderCount(n=>Math.max(0,n-1));
  };

  /* ── Бронирование из прайс-листа ── */
  const [priceBookItem,setPriceBookItem]=useState<PriceItem|null>(null);
  const [priceBookName,setPriceBookName]=useState('');
  const [priceBookPhone,setPriceBookPhone]=useState('');
  const [priceBookStatus,setPriceBookStatus]=useState<'idle'|'sending'|'done'|'error'>('idle');

  const confirmPriceBook=async()=>{
    if(!priceBookItem||!priceBookName.trim()||!priceBookPhone.trim())return;
    setPriceBookStatus('sending');
    try{
      const r=await fetch(`${window.location.origin}/api/booking-request`,{method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({targetHash:userHash,clientName:priceBookName.trim(),
          clientPhone:priceBookPhone.trim(),service:priceBookItem.name,
          price:priceBookItem.price,text:`Запись: ${priceBookItem.name}. Цена: ${priceBookItem.price}. Канал: ${ch.name}`})});
      if(r.ok){
        setPriceBookStatus('done');
        window.dispatchEvent(new CustomEvent('sw-new-booking',{detail:{name:priceBookName.trim(),phone:priceBookPhone.trim(),service:priceBookItem.name}}));
        setTimeout(()=>setPriceBookItem(null),2200);
      }else{setPriceBookStatus('error');}
    }catch{setPriceBookStatus('error');}
  };
    ...
    <div style={{flex:1}}>
      ...
      <div style={{position:'relative',height:160,overflow:'visible',background:ch.coverGradient}}>
        ...
        <div style={{position:'absolute',top:14,right:14,zIndex:2,display:'flex',gap:6}}>
          ...
          {/* CRM кнопка — только если есть прайс-лист или пришли заказы */}
          {((ch.priceList??[]).length>0||channelOrders.length>0)&&(
            <button onClick={()=>setShowChannelCRM(true)}
              style={{position:'relative',display:'flex',alignItems:'center',gap:5,padding:'5px 10px',borderRadius:20,
                background:'rgba(16,185,129,0.45)',border:'1px solid rgba(52,211,153,0.6)',
                color:'#fff',cursor:'pointer',backdropFilter:'blur(8px)',fontSize:11,fontWeight:700}}>
              {newOrderCount>0?(
                <motion.span animate={{scale:[1,1.25,1]}} transition={{repeat:Infinity,duration:0.8}}
                  style={{position:'absolute',top:-5,right:-5,minWidth:18,height:18,borderRadius:9,
      ...
      <ChannelInvitePanel ch={ch} c={c} accent={accent} onUpdate={onUpdate} userName={userName}/>
      ...
      {ch.priceList&&ch.priceList.length>0&&ch.templateId&&(
        <ChannelPriceList
          ...
          onBook={item=>{setPriceBookItem(item);setPriceBookName('');setPriceBookPhone('');setPriceBookStatus('idle');}}
        />
      )}
      ...
      <div style={{padding:'12px 16px'}}>
        ...
        {sortedPosts.map(post=>(
          <PostCard key={post.id} post={post} ch={ch} c={c} accent={accent} tick={tick}
            ...
            onBook={(time,cName,cPhone)=>onBook(post.id,time,cName,cPhone)}
      ...
      {/* ── Модал: Запись из прайса ── */}
      <AnimatePresence>
        {priceBookItem&&(
          <>
            <motion.div key="pbbg" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
              onClick={()=>priceBookStatus!=='sending'&&setPriceBookItem(null)}
              style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(8px)',zIndex:8000}}/>
            <motion.div key="pbsheet" initial={{y:'100%'}} animate={{y:0}} exit={{y:'100%'}}
              ...
              {priceBookStatus==='done'?(
                <div style={{textAlign:'center',padding:'24px 0'}}>
                  <div style={{fontSize:44,marginBottom:12}}>🎉</div>
                  <div style={{fontSize:18,fontWeight:900,color:'#34d399',fontFamily:'"Montserrat",sans-serif'}}>Заявка отправлена!</div>
                  <div style={{fontSize:13,color:'rgba(255,255,255,0.5)',marginTop:6}}>Ждём вас · {priceBookItem.name}</div>
                </div>
              ):(
                <>
                  <div style={{display:'flex',alignItems:'center',gap:10}}>
                    <span style={{fontSize:28}}>{priceBookItem.emoji||'📋'}</span>
                    <div style={{flex:1}}>
                      <div style={{fontSize:15,fontWeight:900,color:'#fff',fontFamily:'"Montserrat",sans-serif'}}>{priceBookItem.name}</div>
                      <div style={{fontSize:13,fontWeight:800,color:ch.vibeColor||accent}}>{priceBookItem.price}</div>
                    </div>
                    <button onClick={()=>setPriceBookItem(null)}
                  ...
                  <p style={{margin:0,fontSize:12,color:'rgba(255,255,255,0.4)',fontWeight:700}}>ЗАПИСАТЬСЯ · {ch.name}</p>
                  <input value={priceBookName} onChange={e=>setPriceBookName(e.target.value)}
                  ...
                  <input value={priceBookPhone} onChange={e=>setPriceBookPhone(e.target.value)}
                    placeholder="Телефон +7… *" type="tel"
                    style={{padding:'12px 14px',borderRadius:14,border:'1px solid rgba(255,255,255,0.12)',
                      background:'rgba(255,255,255,0.05)',color:'#fff',fontSize:14,fontFamily:'inherit',outline:'none'}}/>
                  {priceBookStatus==='error'&&(
                    <p style={{margin:0,fontSize:12,color:'#f87171',textAlign:'center'}}>Ошибка. Попробуйте ещё раз.</p>
                  )}
                  <motion.button whileTap={{scale:0.96}} onClick={confirmPriceBook}
                    disabled={priceBookStatus==='sending'||!priceBookName.trim()||!priceBookPhone.trim()}
                    style={{padding:'14px',borderRadius:16,border:'none',cursor:'pointer',
                      background:(!priceBookName.trim()||!priceBookPhone.trim()||priceBookStatus==='sending')
                        ?'rgba(255,255,255,0.1)':`linear-gradient(135deg,${ch.vibeColor||accent}cc,${ch.vibeColor||accent})`,
                      color:'#fff',fontSize:15,fontWeight:900,fontFamily:'"Montserrat",sans-serif',
                      opacity:(!priceBookName.trim()||!priceBookPhone.trim())?0.5:1}}>
                    {priceBookStatus==='sending'?'Отправляем…':'📅 Записаться'}
                  </motion.button>
                </>
              )}
            </motion.div>
      ...
      {/* ── CRM-панель заказов канала ── */}
      <AnimatePresence>
        {showChannelCRM&&(
          <>
            <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
              onClick={()=>setShowChannelCRM(false)}
              style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(10px)',zIndex:9100}}/>
            <motion.div initial={{y:'100%'}} animate={{y:0}} exit={{y:'100%'}}
              transition={{type:'spring',stiffness:290,damping:30}}
              style={{position:'fixed',bottom:0,left:0,right:0,zIndex:9101,
                background:'linear-gradient(180deg,#060d0f 0%,#040a0c 100%)',
                borderRadius:'28px 28px 0 0',border:'1px solid rgba(52,211,153,0.2)',
                maxHeight:'85vh',display:'flex',flexDirection:'column'}}>
              {/* Шапка CRM */}
              <div style={{padding:'16px 20px 12px',borderBottom:'1px solid rgba(52,211,153,0.15)',flexShrink:0,display:'flex',alignItems:'center',gap:12}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:16,fontWeight:900,color:'#fff',fontFamily:'"Montserrat",sans-serif'}}>
                    📋 CRM · {ch.name}
                  </div>
                  <div style={{fontSize:11,color:'rgba(52,211,153,0.7)',marginTop:2}}>
                    {channelOrders.length} {channelOrders.length===1?'заявка':channelOrders.length>4?'заявок':'заявки'}
                    {newOrderCount>0&&<span style={{marginLeft:8,color:'#ef4444',fontWeight:800}}>· {newOrderCount} новых</span>}
                  </div>
                </div>
                <motion.button whileTap={{scale:0.9}} onClick={fetchChannelOrders}
                  style={{width:36,height:36,borderRadius:10,background:'rgba(52,211,153,0.1)',border:'1px solid rgba(52,211,153,0.3)',
                    color:'#34d399',cursor:'pointer',fontSize:16,display:'flex',alignItems:'center',justifyContent:'center'}}>
                  {channelOrdersLoading?<motion.span animate={{rotate:360}} transition={{repeat:Infinity,duration:0.8,ease:'linear'}}>↻</motion.span>:'↻'}
                </motion.button>
                <motion.button whileTap={{scale:0.9}} onClick={()=>setShowChannelCRM(false)}
                  style={{width:36,height:36,borderRadius:10,background:'rgba(255,255,255,0.07)',border:'none',
                    color:'rgba(255,255,255,0.5)',cursor:'pointer',fontSize:16}}>✕</motion.button>
              </div>
              {/* Список заказов */}
              <div style={{overflowY:'auto',flex:1,padding:'12px 16px 24px'}}>
                {channelOrdersLoading&&channelOrders.length===0?(
                  <div style={{textAlign:'center',padding:'40px 0',color:'rgba(255,255,255,0.4)',fontSize:14}}>Загрузка…</div>
                ):channelOrders.length===0?(
                  <div style={{textAlign:'center',padding:'40px 0'}}>
                    <div style={{fontSize:36,marginBottom:10}}>📭</div>
                    <div style={{fontSize:14,color:'rgba(255,255,255,0.4)'}}>Заявок пока нет</div>
                    <div style={{fontSize:11,color:'rgba(255,255,255,0.25)',marginTop:4}}>Они появятся когда гости закажут услугу</div>
                  </div>
                ):(
                  channelOrders.slice().reverse().map((order:any)=>{
                    const isNew=order.status==='new';
                    const isDone=order.status==='done';
                    const isCancelled=order.status==='cancelled';
                    const statusColor=isNew?'#fbbf24':isDone?'#34d399':'#6b7280';
                    const statusLabel=isNew?'Новая':isDone?'Выполнена':'Отменена';
                    return(
                      <motion.div key={order.id} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}
                        style={{borderRadius:16,background:isNew?'rgba(251,191,36,0.06)':'rgba(255,255,255,0.03)',
                          border:`1px solid ${isNew?'rgba(251,191,36,0.25)':isDone?'rgba(52,211,153,0.15)':'rgba(255,255,255,0.07)'}`,
                          padding:'14px 16px',marginBottom:10}}>
                        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:10,marginBottom:8}}>
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{fontSize:14,fontWeight:900,color:'#fff',marginBottom:2}}>{order.service||'Заявка'}</div>
                            <div style={{fontSize:12,color:'rgba(255,255,255,0.6)'}}>
                              {order.clientName} · {order.clientPhone}
                            </div>
                            {order.price&&<div style={{fontSize:13,fontWeight:800,color:accent,marginTop:2}}>{order.price}</div>}
                            {order.slot&&<div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginTop:2}}>📅 {order.slot}</div>}
                          </div>
                          <div style={{flexShrink:0,textAlign:'right'}}>
                            <div style={{fontSize:10,color:statusColor,fontWeight:800,background:`${statusColor}18`,
                              borderRadius:20,padding:'3px 10px',border:`1px solid ${statusColor}44`}}>
                              {statusLabel}
                            </div>
                        ...
                        {isNew&&(
                          <div style={{display:'flex',gap:8,marginTop:4}}>
                            <motion.button whileTap={{scale:0.94}} onClick={()=>updateChannelOrderStatus(order.id,'done')}
                              style={{flex:1,padding:'8px 0',borderRadius:10,border:'1px solid rgba(52,211,153,0.4)',
                                background:'rgba(52,211,153,0.12)',color:'#34d399',cursor:'pointer',fontSize:12,fontWeight:800}}>
                              ✓ Принять
                            </motion.button>
                            <motion.button whileTap={{scale:0.94}} onClick={()=>updateChannelOrderStatus(order.id,'cancelled')}
                              style={{flex:1,padding:'8px 0',borderRadius:10,border:'1px solid rgba(239,68,68,0.3)',
                                background:'rgba(239,68,68,0.08)',color:'#f87171',cursor:'pointer',fontSize:12,fontWeight:800}}>
                              ✕ Отклонить
                            </motion.button>
                          </div>
                        )}
                        {isDone&&(
                          <div style={{fontSize:11,color:'#34d399',marginTop:4}}>✓ Принято · работа завершена</div>
                        )}
                      </motion.div>
                    );
                  })
...
function PostCard({post,ch,c,accent,tick,onReact,onVote,onOpenCapsule,onPin,onDelete,onBook,onEditBooking,ownerHash}:{
  ...
  onBook:(time?:string,clientName?:string,clientPhone?:string)=>void;
    ...
    <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}}
      ...
      {showActions&&(
        <div style={{display:'flex',gap:6,padding:'0 14px 10px',flexWrap:'wrap'}}>
          ...
          {ownerHash&&(
            <button onClick={()=>{
              const id=btoa(`${ownerHash}:${post.id}`);
              const url=`${window.location.origin}/post/${id}`;
              setShareUrl(url);setShareCopied(false);setShowActions(false);
              /* Синхронизируем каналы, чтобы пост был доступен на сервере */
              try{
                const tok=localStorage.getItem('swaip_session')||'';
                const key=`swaip_account_${ownerHash}_channels_v2`;
                const channels=JSON.parse(localStorage.getItem(key)||'[]');
                if(tok&&channels.length){
                  fetch(`${window.location.origin}/api/channels-sync`,{method:'PUT',
                    headers:{'Content-Type':'application/json','x-session-token':tok},
                    body:JSON.stringify({channels})}).catch(()=>{});
                }
              }catch{}
            }} style={{flex:1,minWidth:90,padding:'7px',borderRadius:10,
      ...
      {shareUrl&&(
        <div style={{margin:'0 14px 10px',padding:'10px 12px',borderRadius:12,
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:10,color:'rgba(96,165,250,0.7)',fontWeight:700,marginBottom:2}}>ССЫЛКА НА ПОСТ</div>
            <div style={{fontSize:11,color:'#60a5fa',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{shareUrl}</div>
          </div>
          <button onClick={()=>{
            try{navigator.clipboard.writeText(shareUrl);setShareCopied(true);setTimeout(()=>setShareCopied(false),2000);}catch{}
          }} style={{padding:'6px 10px',borderRadius:8,border:'1px solid rgba(59,130,246,0.4)',
            background:shareCopied?'rgba(16,185,129,0.2)':'rgba(59,130,246,0.15)',
            color:shareCopied?'#34d399':'#60a5fa',fontSize:11,fontWeight:800,cursor:'pointer',flexShrink:0}}>
            {shareCopied?'✓ Скопировано':'Копировать'}
          </button>
          <button onClick={()=>setShareUrl('')}
            style={{background:'none',border:'none',color:'rgba(255,255,255,0.3)',cursor:'pointer',fontSize:14,padding:2,flexShrink:0}}>✕</button>
        </div>
      )}
      ...
      {post.hasBooking&&(freeSlots.length>0||!!bookedTime)&&(
        ...
        setTimeout(()=>{
          ...
          if(chChatStep==='chat'){
            ...
            if(!chChatName){
              ...
              setBookedTime(slot||'✓');setShowChChat(false);onBook(slot||'✓',chChatName,text.trim());
  ...
  const uploadVoice=async(blob:Blob):Promise<string>=>{
    try{
      const r=await fetch(`${window.location.origin}/api/audio-upload`,{
        method:'POST',headers:{'Content-Type':blob.type||'audio/webm','x-session-token':_getTok()},body:blob});
      if(r.ok){const{url}=await r.json();return url;}
    }catch{}
