    ...
    const handler = (e: MessageEvent) => {
      try {
        ...
        if (msg.type === 'mute_request') {
          localParticipant?.setMicrophoneEnabled(false).catch(() => {});
          setMicEnabled(false);
          showToast('🔇 Ведущий отключил ваш микрофон');
        }
        ...
        {ctxMenu && (
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.92 }}
              ...
              <CtxItem icon="🔇" label="Отключить микрофон" onClick={() => {
                const ws = wsRef.current;
                if (ws && ws.readyState === WebSocket.OPEN) {
                  ws.send(JSON.stringify({ type: 'mute_request', targetId: ctxMenu.targetId }));
                  showToast('🔇 Запрос на отключение отправлен');
                }
                closeCtx();
              }} />
