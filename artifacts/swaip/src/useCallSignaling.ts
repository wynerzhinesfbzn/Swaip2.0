...
export type RingtoneId =
  | 'classic' | 'modern' | 'digital' | 'pulse'
  | 'marimba' | 'crystal' | 'cosmic' | 'jazz' | 'urgent' | 'harmony'
  | 'custom';
...
export const RINGTONE_OPTIONS: { id: RingtoneId; name: string; emoji: string }[] = [
  ...
  { id: 'marimba',  name: 'Маримба',       emoji: '🪘' },
  { id: 'crystal',  name: 'Хрустальный',   emoji: '💎' },
  { id: 'harmony',  name: 'Гармония',      emoji: '🎶' },
  { id: 'jazz',     name: 'Джаз',          emoji: '🎷' },
  { id: 'cosmic',   name: 'Космик',        emoji: '🚀' },
  { id: 'digital',  name: 'Цифровой',      emoji: '💻' },
  { id: 'pulse',    name: 'Пульс',         emoji: '💓' },
  { id: 'urgent',   name: 'Срочный',       emoji: '🔴' },
...
class SynthRing {
  ...
  private loop() {
    ...
    const intervals: Record<string, number> = {
      classic: 5000, modern: 4200, digital: 2800, pulse: 2000,
      marimba: 4500, crystal: 4000, cosmic: 3800, jazz: 5200, urgent: 1800, harmony: 5500,
    };
  ...
  /* Быстрое затухание — удар маримбы/ксилофона */
  private strike(freq: number, start: number, vol = 0.45) {
    const ctx = this.ctx!;
    const osc = ctx.createOscillator();
    const g   = ctx.createGain();
    osc.connect(g); g.connect(ctx.destination);
  ...
  /* Chord — несколько осцилляторов одновременно */
  private chord(freqs: number[], start: number, dur: number, vol = 0.22) {
    freqs.forEach(f => this.tone(f, start, dur, 'sine', vol));
  }

  private playPattern() {
    switch (this.kind) {

      /* 1 ─ Классический: двухтональный телефонный звонок */
      case 'classic':
        this.tone(480, 0,   1.8, 'square', 0.28);
        this.tone(440, 0,   1.8, 'square', 0.28);
        break;

      /* 2 ─ Современный: трель вверх */
      case 'modern':
        [880, 1100, 1320].forEach((f, i) => this.tone(f, i * 0.22, 0.18, 'sine', 0.38));
        this.tone(1760, 0.75, 0.28, 'sine', 0.3);
        break;

      /* 3 ─ Маримба: ксилофонная мелодия до-ми-соль-до */
      case 'marimba':
        [523, 659, 784, 1046, 784, 659, 523].forEach((f, i) =>
          this.strike(f, i * 0.28, 0.44));
        break;

      /* 4 ─ Хрустальный: высокие чистые синусоиды с эхом */
      case 'crystal':
        [2093, 1760, 2093, 2637].forEach((f, i) => {
          this.tone(f, i * 0.32, 0.26, 'sine', 0.32);
          this.tone(f / 2, i * 0.32, 0.26, 'sine', 0.10); /* октава ниже */
        });
        break;

      /* 5 ─ Гармония: мягкие аккорды до-фа-соль */
      case 'harmony':
        this.chord([523, 659, 784],  0,    0.8, 0.20);
        this.chord([698, 880, 1047], 0.95, 0.8, 0.20);
        this.chord([784, 988, 1175], 1.90, 0.8, 0.20);
        this.chord([523, 659, 784],  2.85, 1.0, 0.22);
        break;

      /* 6 ─ Джаз: свингующий пентатонический мотив */
      case 'jazz':
        [392, 440, 523, 440, 392, 349, 392, 440].forEach((f, i) =>
          this.tone(f, i * 0.19, 0.14, 'triangle', 0.34));
        this.tone(523, 1.6, 0.4, 'triangle', 0.28);
        break;

      /* 7 ─ Космик: синтезаторный sci-fi звук с нарастанием */
      case 'cosmic':
        for (let i = 0; i < 6; i++) {
          this.tone(220 + i * 110, i * 0.22, 0.18, 'sawtooth', 0.18);
          this.tone(440 + i * 55,  i * 0.22, 0.18, 'square',   0.08);
        }
        this.tone(1760, 1.4, 0.5, 'sine', 0.28);
        break;

      /* 8 ─ Цифровой: быстрые короткие пики */
      case 'digital':
        for (let i = 0; i < 5; i++) this.tone(1200 + i * 60, i * 0.18, 0.12, 'square', 0.22);
        break;

      /* 9 ─ Пульс: три низкочастотных импульса */
      case 'pulse':
        [0, 0.55, 1.1].forEach(t => this.tone(620, t, 0.35, 'sine', 0.42));
        break;

      /* 10 ─ Срочный: тревожные частые бипы */
      case 'urgent':
        for (let i = 0; i < 4; i++) {
          this.tone(1400, i * 0.22, 0.12, 'square', 0.30);
          this.tone(1200, i * 0.22 + 0.12, 0.06, 'square', 0.18);
        }
        break;
    }
  }
