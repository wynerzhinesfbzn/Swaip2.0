    ...
    <div style={{ padding: '14px 16px' }}>
      ...
      {editing ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {filtered.map(item => (
              <div key={item.id} style={{ borderRadius: 14, background: c.card || 'rgba(255,255,255,0.04)', border: `1px solid ${c.border}`, overflow: 'hidden' }}>
                ...
                {onBook && type === 'services' && (
                  <button onClick={() => onBook(item)}
                    style={{ width: '100%', padding: '8px', border: 'none', borderTop: `1px solid ${color}25`,
                      background: `${color}12`, color, fontSize: 12, fontWeight: 800, cursor: 'pointer',
                      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                    <span>📅</span><span>Записаться</span>
                  </button>
                )}
              </div>
            ))}
