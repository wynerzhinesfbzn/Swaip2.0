...
export type ScreenType = 'text' | 'image' | 'video' | 'action';
export interface BotScreen {
  ...
  x: number;
  y: number;
  /** Задержка перед показом этого экрана (мс). 0 = сразу */
  delayMs?: number;
  /** Для type='action': счётчик от */
  actionFrom?: number;
  /** Для type='action': счётчик до */
  actionTo?: number;
  /** Для type='action': длительность анимации (мс) */
  actionDuration?: number;
  /** Для type='action': ID экрана для авто-перехода после анимации */
  autoNext?: string;
}
...
function getSessionToken() { return localStorage.getItem('swaip_session') || localStorage.getItem('swaip_session_token') || ''; }

function makeScreen(overrides: Partial<BotScreen> = {}): BotScreen {
  return { id: uid(), name: 'Новый экран', text: '', imageUrl: '', videoUrl: '', videoSkin: 'swaip', type: 'text', buttons: [], x: 0, y: 0, delayMs: 0, actionFrom: 1, actionTo: 10, actionDuration: 3000, autoNext: '', ...overrides };
}
...
/* ═══════════════════════════════════════════════════════════
   БОТ-ЧАТ — накопительный чат-вид (Telegram/VK стиль)
═══════════════════════════════════════════════════════════ */

/** Одно сообщение в истории чата */
interface ChatHistoryItem {
  screenId: string;
  /** ID кнопки, которую нажал пользователь (если нажал) */
  chosenBtnId?: string;
}

/** Компонент: анимированный счётчик для type='action' */
function ActionCounter({ screen, onDone }: { screen: BotScreen; onDone: () => void }) {
  const from = screen.actionFrom ?? 1;
  const to = screen.actionTo ?? 10;
  const dur = screen.actionDuration ?? 3000;
  const [cur, setCur] = useState(from);
  const [done, setDone] = useState(false);

  useEffect(() => {
    const steps = Math.abs(to - from);
    if (steps === 0) { setTimeout(onDone, 600); return; }
  ...
  const progress = Math.abs(to - from) === 0 ? 1 : Math.abs(cur - from) / Math.abs(to - from);

  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'flex-start',gap:8,maxWidth:'85%'}}>
      <div style={{background:C.card,border:`1px solid ${done?GREEN:C.border}`,borderRadius:'4px 16px 16px 16px',padding:'16px 24px',minWidth:120,transition:'border-color 0.3s'}}>
        <div style={{fontSize:11,color:C.sub,marginBottom:6,fontWeight:600}}>⚡ Действие</div>
        <motion.div
          key={cur}
          initial={{scale:0.5,opacity:0,y:10}}
          animate={{scale:1,opacity:1,y:0}}
          transition={{type:'spring',stiffness:500,damping:22}}
          style={{fontSize:52,fontWeight:900,color:done?GREEN:AC,lineHeight:1,textAlign:'center',fontVariantNumeric:'tabular-nums'}}
        >
          {cur}
        </motion.div>
        <div style={{marginTop:10,height:4,borderRadius:4,background:C.border,overflow:'hidden'}}>
          <motion.div
            animate={{width:`${progress*100}%`}}
            transition={{ease:'linear',duration:0.1}}
            style={{height:'100%',background:done?GREEN:AC,borderRadius:4}}
          />
        </div>
        <div style={{marginTop:4,fontSize:10,color:C.sub,textAlign:'center'}}>{from} → {to}</div>
      </div>
      {done && (
        <motion.div initial={{opacity:0,y:4}} animate={{opacity:1,y:0}} style={{fontSize:11,color:GREEN,paddingLeft:4}}>
          ✓ Готово
        </motion.div>
      )}
    </div>
...
/** Одна строка истории чата */
function ChatBubble({
  sc, bot, cfg, chosenBtnId, onChooseBtn, isLast
}: {
  sc: BotScreen; bot: SwBot; cfg: BotConfig;
  chosenBtnId?: string; onChooseBtn?: (btnId: string, nextId: string) => void; isLast: boolean;
}) {
  const ytId = sc.type==='video' ? getYouTubeId(sc.videoUrl||'') : null;
  const delayLabel = sc.delayMs ? formatDelay(sc.delayMs) : null;
  ...
  return (
    ...
    >
      {/* Метка задержки */}
      {delayLabel && (
        <div style={{textAlign:'center',fontSize:11,color:C.sub,padding:'2px 0'}}>
          <span style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:'3px 12px',display:'inline-block'}}>
            🕐 {delayLabel}
          </span>
        </div>
      )}
      ...
      <div style={{display:'flex',gap:10,alignItems:'flex-start'}}>
        <div style={{width:36,height:36,borderRadius:'50%',background:AC+'33',border:`2px solid ${AC}55`,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,overflow:'hidden',marginTop:2}}>
          {bot.avatarUrl ? <img src={bot.avatarUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/> : '🤖'}
        </div>
        <div style={{flex:1,minWidth:0}}>
          {/* Action: завершённое (в истории — уже выполнено) */}
          {sc.type==='action' && !isLast && (
            <div style={{background:C.card,border:`1px solid ${GREEN}55`,borderRadius:'4px 16px 16px 16px',padding:'10px 16px',display:'inline-flex',alignItems:'center',gap:10}}>
              <div style={{fontSize:26,fontWeight:900,color:GREEN}}>{sc.actionTo??10}</div>
              <div>
                <div style={{fontSize:11,color:GREEN,fontWeight:700}}>⚡ Выполнено</div>
                <div style={{fontSize:10,color:C.sub}}>{sc.actionFrom??1} → {sc.actionTo??10}</div>
              </div>
            </div>
          )}
          {/* Обычный контент */}
          {sc.type!=='action' && (
            <>
              {sc.type==='video' && sc.videoUrl && (
                <div style={{marginBottom:8,maxWidth:300}}>
                  {ytId
                    ? <div style={{borderRadius:14,overflow:'hidden',background:'#000'}}>
                        <iframe src={`https://www.youtube.com/embed/${ytId}`} style={{width:'100%',height:180,border:'none',display:'block'}} allowFullScreen title="video"/>
                      </div>
                    : <SwaipVideoPlayer src={sc.videoUrl} skin={sc.videoSkin||'swaip'} title={sc.name}/>}
                </div>
              )}
              {sc.type==='image' && sc.imageUrl && (
                <div style={{borderRadius:14,overflow:'hidden',marginBottom:8,maxWidth:280}}>
                  <img src={sc.imageUrl} alt="" style={{width:'100%',display:'block',maxHeight:220,objectFit:'cover'}}/>
                </div>
              )}
              {sc.text && (
                <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:'4px 16px 16px 16px',padding:'10px 14px',maxWidth:300,wordBreak:'break-word'}}>
                  <div style={{fontSize:14,color:C.light,lineHeight:1.65,whiteSpace:'pre-wrap'}}>{sc.text}</div>
                </div>
              )}
            </>
          )}
      ...
      {/* Кнопки — только если это последний пузырь И есть кнопки */}
      {isLast && sc.buttons.length > 0 && onChooseBtn && (
        <div style={{display:'flex',flexDirection:'column',gap:7,paddingLeft:46}}>
          {sc.buttons.map(btn => {
            const isChosen = chosenBtnId === btn.id;
            const valid = !!btn.nextScreen && !!cfg.screens[btn.nextScreen];
            return (
              <motion.button key={btn.id} whileTap={{scale:0.96}}
                onClick={() => valid && onChooseBtn(btn.id, btn.nextScreen)}
                disabled={!valid}
                style={{padding:'11px 16px',borderRadius:12,
                  background:isChosen?AC:valid?AC+'18':'rgba(0,0,0,0.1)',
                  border:`1.5px solid ${isChosen?AC:valid?AC+'55':C.border}`,
                  color:isChosen?'#fff':valid?AC:C.sub,fontWeight:700,fontSize:13,
                  cursor:valid?'pointer':'not-allowed',textAlign:'left',
                  fontFamily:'inherit',transition:'all 0.15s'}}>
                {btn.label||'—'}
                {!valid&&<span style={{fontSize:10,opacity:0.45,marginLeft:6}}>(не привязана)</span>}
              </motion.button>
            );
          })}
        </div>
      ...
      {/* Выбранная кнопка (в истории — показываем что нажал пользователь) */}
      {!isLast && chosenBtnId && sc.buttons.length > 0 && (
        <div style={{display:'flex',flexDirection:'row-reverse',paddingRight:4}}>
          <div style={{padding:'9px 16px',borderRadius:'16px 4px 16px 16px',background:AC+'28',border:`1.5px solid ${AC}55`,color:AC,fontSize:13,fontWeight:700,maxWidth:220}}>
            {sc.buttons.find(b=>b.id===chosenBtnId)?.label||'—'}
          </div>
        </div>
      )}

      {/* Конец диалога — нет кнопок, нет автоперехода, последний экран */}
      {isLast && sc.buttons.length === 0 && sc.type!=='action' && (
...
export function BotChat({ bot, onClose }: { bot: SwBot; onClose: () => void }) {
  ...
  const startId = cfg.startScreen || Object.keys(cfg.screens)[0] || '';

  const [history, setHistory] = useState<ChatHistoryItem[]>([{ screenId: startId }]);
  const [awaitingDelay, setAwaitingDelay] = useState(false);
  ...
  const currentScreen = cfg.screens[currentItem?.screenId];

  useEffect(() => {
    bottomRef.current?.scrollIntoView({ behavior: 'smooth' });
  }, [history, awaitingDelay, actionRunning]);

  /* Инициируем переход на следующий экран (с задержкой или сразу) */
  const goNext = useCallback((nextId: string) => {
    ...
    const next = cfg.screens[nextId];
    const delay = next.delayMs || 0;
    const previewDelay = Math.min(delay / 10, 3000); // в превью — сжатое время (макс 3с)

    if (previewDelay > 0) {
      setAwaitingDelay(true);
      setPendingNextId(nextId);
      setTimeout(() => {
        setAwaitingDelay(false);
        setPendingNextId(null);
        setHistory(h => [...h, { screenId: nextId }]);
        if (next.type === 'action') setActionRunning(true);
      }, previewDelay);
    } else {
      setHistory(h => [...h, { screenId: nextId }]);
      if (next.type === 'action') setActionRunning(true);
  ...
  const handleBtn = useCallback((btnId: string, nextId: string) => {
    setHistory(h => h.map((item, i) => i === h.length - 1 ? { ...item, chosenBtnId: btnId } : item));
    goNext(nextId);
  }, [goNext]);

  /* Действие (action) завершилось — авто-переход */
  const handleActionDone = useCallback(() => {
    setActionRunning(false);
    const sc = cfg.screens[currentItem?.screenId];
    if (sc?.autoNext && cfg.screens[sc.autoNext]) {
      goNext(sc.autoNext);
    }
  }, [cfg, currentItem, goNext]);

  if (!currentScreen && history.length <= 1) return (
  ...
  return (
    <div style={{position:'fixed',inset:0,background:C.deep,display:'flex',flexDirection:'column',zIndex:1100,fontFamily:'inherit'}}>
      ...
      <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:'10px 16px',display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
        ...
        <div style={{width:38,height:38,borderRadius:'50%',background:AC+'33',border:`2px solid ${AC}55`,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,overflow:'hidden'}}>
          {bot.avatarUrl ? <img src={bot.avatarUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/> : '🤖'}
        </div>
        <div style={{flex:1}}>
          <div style={{fontSize:14,fontWeight:800,color:C.light}}>{bot.name}</div>
          <div style={{fontSize:11,color:awaitingDelay?ORANGE:GREEN}}>
            {awaitingDelay ? '⏳ Печатает...' : '🟢 Онлайн'}
          </div>
        </div>
        <div style={{fontSize:10,color:C.sub,background:C.cardAlt,border:`1px solid ${C.border}`,borderRadius:8,padding:'4px 8px'}}>
          {history.length} / {Object.keys(cfg.screens).length}
        </div>
      </div>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'16px 14px',display:'flex',flexDirection:'column',gap:14}}>
        {history.map((item, idx) => {
          const sc = cfg.screens[item.screenId];
          if (!sc) return null;
          const isLast = idx === history.length - 1;
          return (
            <div key={`${item.screenId}-${idx}`}>
              {isLast && sc.type === 'action' ? (
                <div style={{display:'flex',gap:10,alignItems:'flex-start'}}>
                  <div style={{width:36,height:36,borderRadius:'50%',background:AC+'33',border:`2px solid ${AC}55`,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,overflow:'hidden',marginTop:2}}>
                    {bot.avatarUrl ? <img src={bot.avatarUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/> : '🤖'}
                  </div>
                  {sc.delayMs ? (
                    <div style={{textAlign:'center',fontSize:11,color:C.sub,padding:'2px 0'}}>
                      <span style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:'3px 12px',display:'inline-block'}}>
                        🕐 {formatDelay(sc.delayMs)}
                      </span>
                    </div>
                  ) : null}
                  <ActionCounter screen={sc} onDone={handleActionDone}/>
                </div>
              ) : (
                <ChatBubble
                  sc={sc} bot={bot} cfg={cfg}
                  chosenBtnId={item.chosenBtnId}
                  isLast={isLast && !awaitingDelay}
                  onChooseBtn={isLast && !awaitingDelay ? handleBtn : undefined}
                />
              )}
            </div>
          );
        ...
        {awaitingDelay && (
          <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} style={{display:'flex',gap:10,alignItems:'flex-end'}}>
            <div style={{width:36,height:36,borderRadius:'50%',background:AC+'33',border:`2px solid ${AC}55`,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,overflow:'hidden'}}>
              {bot.avatarUrl ? <img src={bot.avatarUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/> : '🤖'}
            </div>
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:'4px 16px 16px 16px',padding:'12px 18px',display:'flex',gap:5,alignItems:'center'}}>
              {[0,1,2].map(i=>(
                <motion.div key={i} animate={{y:[0,-6,0]}} transition={{repeat:Infinity,duration:0.8,delay:i*0.15}}
                  style={{width:7,height:7,borderRadius:'50%',background:C.mid}}/>
              ))}
            </div>
            {pendingNextId && cfg.screens[pendingNextId]?.delayMs ? (
              <div style={{fontSize:10,color:ORANGE,alignSelf:'center',paddingLeft:4}}>
                {formatDelay(cfg.screens[pendingNextId]!.delayMs!)}
              </div>
            ) : null}
          </motion.div>
        )}
...
function ScreenEditor({
  ...
  const TYPE_OPTS: { type: ScreenType; icon: string; label: string }[] = [
    ...
    { type:'action', icon:'⚡', label:'Действие' },
  ];

  const DELAY_OPTS: { label: string; ms: number }[] = [
    { label:'Сразу', ms:0 },
    { label:'5 мин', ms:300_000 },
    { label:'30 мин', ms:1_800_000 },
    { label:'1 час', ms:3_600_000 },
    { label:'3 часа', ms:10_800_000 },
    { label:'6 часов', ms:21_600_000 },
    { label:'12 часов', ms:43_200_000 },
    { label:'1 день', ms:86_400_000 },
    { label:'2 дня', ms:172_800_000 },
    { label:'3 дня', ms:259_200_000 },
    { label:'7 дней', ms:604_800_000 },
  ];
    ...
    <div style={{flex:1,overflowY:'auto',padding:'14px',display:'flex',flexDirection:'column',gap:14}}>
      ...
      <div>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
          {DELAY_OPTS.map(o=>{
            const active = (screen.delayMs||0) === o.ms;
            return (
              <button key={o.ms} onClick={()=>set({delayMs:o.ms})}
                style={{padding:'6px 11px',borderRadius:8,background:active?ORANGE:C.cardAlt,
                  border:`1.5px solid ${active?ORANGE:C.border}`,color:active?'#fff':C.mid,
                  fontSize:11,fontWeight:active?800:600,cursor:'pointer',fontFamily:'inherit',transition:'all 0.15s'}}>
                {o.label}
              </button>
            );
          })}
        ...
        {(screen.delayMs||0) > 0 && (
          <div style={{marginTop:6,fontSize:11,color:ORANGE,padding:'5px 10px',background:ORANGE+'12',borderRadius:8,border:`1px solid ${ORANGE}33`}}>
            📬 Это сообщение придёт {formatDelay(screen.delayMs!)} после предыдущего
          </div>
        )}
      </div>

      {/* Настройки для типа Действие */}
      {screen.type==='action' && (
        <div style={{background:C.card,border:`1.5px solid ${AC}44`,borderRadius:12,padding:'12px',display:'flex',flexDirection:'column',gap:10}}>
          <div style={{fontSize:11,fontWeight:800,color:AC}}>⚡ Настройки анимации-счётчика</div>
          <div style={{display:'flex',gap:8}}>
            <div style={{flex:1}}>
              <div style={{fontSize:10,color:C.sub,marginBottom:4,fontWeight:600}}>От</div>
              <input type="number" value={screen.actionFrom??1} onChange={e=>set({actionFrom:Number(e.target.value)})}
                style={{...inp(),textAlign:'center'}}/>
            </div>
            <div style={{flex:1}}>
              <div style={{fontSize:10,color:C.sub,marginBottom:4,fontWeight:600}}>До</div>
              <input type="number" value={screen.actionTo??10} onChange={e=>set({actionTo:Number(e.target.value)})}
                style={{...inp(),textAlign:'center'}}/>
            </div>
            <div style={{flex:1}}>
              <div style={{fontSize:10,color:C.sub,marginBottom:4,fontWeight:600}}>Длит. (сек)</div>
              <input type="number" value={Math.round((screen.actionDuration??3000)/1000)} min={1} max={30}
                onChange={e=>set({actionDuration:Number(e.target.value)*1000})}
                style={{...inp(),textAlign:'center'}}/>
            </div>
          </div>
          <div>
            <div style={{fontSize:10,color:C.sub,marginBottom:6,fontWeight:600}}>↪ После анимации перейти на</div>
            <motion.button whileTap={{scale:0.98}} onClick={()=>setConnectingBtnId('__autoNext__')}
              style={{width:'100%',padding:'9px 12px',borderRadius:9,background:screen.autoNext&&screens[screen.autoNext]?AC+'22':C.cardAlt,
                border:`1.5px solid ${screen.autoNext&&screens[screen.autoNext]?AC:C.border}`,
                color:screen.autoNext&&screens[screen.autoNext]?AC:C.sub,fontSize:12,fontWeight:700,
                cursor:'pointer',fontFamily:'inherit',textAlign:'left',display:'flex',alignItems:'center',gap:8}}>
              <span style={{fontSize:16}}>↪</span>
              <span style={{flex:1}}>
                {screen.autoNext && screens[screen.autoNext]
                  ? `→ ${screens[screen.autoNext].name}`
                  : 'Выбрать экран для авто-перехода'}
              </span>
              <span style={{fontSize:11,color:C.sub}}>✎</span>
            </motion.button>
          </div>
          <div style={{padding:'8px 10px',background:AC+'10',borderRadius:8,fontSize:11,color:C.mid,lineHeight:1.5}}>
            💡 Счётчик считает от <b style={{color:C.light}}>{screen.actionFrom??1}</b> до <b style={{color:C.light}}>{screen.actionTo??10}</b> за <b style={{color:C.light}}>{Math.round((screen.actionDuration??3000)/1000)} сек</b>, затем автоматически переходит на следующий экран.
          </div>
        </div>
      )}
      ...
      {/* ConnectPicker — для кнопок и для autoNext */}
      <AnimatePresence>
        {connectingBtnId === '__autoNext__' && (
          <ConnectPicker
            screens={screens}
            currentId={screen.id}
            value={screen.autoNext||''}
            onPick={id=>set({autoNext:id})}
            onClose={()=>setConnectingBtnId(null)}
          />
        )}
        {connectingBtnId && connectingBtnId !== '__autoNext__' && connectingBtn && (
          <ConnectPicker
            screens={screens}
            currentId={screen.id}
...
function ScreenCard({
  ...
  const icon = screen.type==='video'?'🎥':screen.type==='image'?'📷':screen.type==='action'?'⚡':'📝';
  const hasDelay = (screen.delayMs||0) > 0;
  return (
    <motion.div whileTap={{scale:0.98}} onClick={onClick}
      ...
      {hasDelay && <div style={{position:'absolute',top:-8,right:8,fontSize:9,fontWeight:800,color:ORANGE,background:C.deep,padding:'1px 7px',borderRadius:8,border:`1px solid ${ORANGE}44`}}>🕐 {formatDelay(screen.delayMs!)}</div>}
      ...
      {screen.type==='action' && (
        <div style={{fontSize:10,color:AC,marginBottom:4,fontWeight:700}}>
          ⚡ Счётчик {screen.actionFrom??1} → {screen.actionTo??10} ({Math.round((screen.actionDuration??3000)/1000)}с)
          {screen.autoNext && screens[screen.autoNext] && <span style={{color:GREEN}}> → {screens[screen.autoNext].name}</span>}
        </div>
      )}
      ...
      {screen.text && screen.type!=='action' && (
...
const HELP_STEPS = [
  ...
  { icon:'📝', title:'Тип контента',
    text:'Каждый экран может быть: 📝 Текст, 📷 Фото, 🎥 Видео, ⚡ Действие (анимированный счётчик). Выберите тип вверху редактора.' },
  { icon:'🕐', title:'Задержка доставки',
    text:'Для каждого экрана можно задать задержку: «через 5 минут», «через 1 день», «через 7 дней». Экран-сообщение придёт пользователю автоматически спустя выбранное время. В превью задержка сжата до нескольких секунд.' },
  { icon:'⚡', title:'Действие — анимация счётчика',
    text:'Тип «Действие» — это анимированный счётчик: например от 1 до 10. Числа меняются с анимацией, а после окончания бот автоматически переходит на следующий экран. Задайте начало, конец и длительность.' },
  { icon:'🔘', title:'Добавить кнопки',
    text:'Нажмите «+ Добавить кнопку». Введите текст кнопки. Затем нажмите «Нажмите, чтобы выбрать экран» — откроется список. Выбранная кнопка остаётся видна в истории чата.' },
  { icon:'⭐', title:'Стартовый экран',
    text:'Первый экран, с которого начинается бот. Нажмите «⭐ Сделать стартовым» в редакторе. Он отображается с меткой СТАРТ в левой колонке.' },
