/* ═══════════════════════════════════════════════════════════════════
   SWAP — Серверная NSFW-проверка изображений через GPT-4o-mini Vision
...
export async function nsfwServerCheck(
  ...
  try {
    ...
    if (!response.ok) {
      logger.warn({ status: response.status }, "[nsfwServerCheck] OpenAI API error — fail-open");
    ...
    if (blocked) {
      logger.info('[nsfwServerCheck] NSFW detected by GPT-4o-mini vision check');
    }

    return { blocked, reason: blocked ? 'nsfw_vision' : undefined };
  } catch (err) {
    logger.warn({ err }, "[nsfwServerCheck] Error calling OpenAI Vision — fail-open");
    return { blocked: false };
  }
}
