...
/*
 * POST /api/account/ensure
 * Instantly registers a minimal account row for the authenticated user if it doesn't exist yet.
 * Uses INSERT ... ON CONFLICT DO NOTHING — never overwrites existing data.
 * Called immediately on app load so the account is discoverable via invite-code search
 * even before the full GET→merge→PUT sync cycle completes.
 * IMPORTANT: must be registered BEFORE /account/:hash to avoid route collision
 */
router.post("/account/ensure", requireSession, async (req, res) => {
  ...
  try {
    ...
    logger.error({ err: e }, '[ensure] account ensure error');
    return res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/invite-code/:code", async (req, res) => {
  ...
  try {
    ...
    logger.error({ err }, '[invite-code] search error');
    return res.status(500).json({ error: "Server error" });
  }
});
...
router.patch("/account/booking-requests/:id", requireSession, async (req, res) => {
  ...
  const VALID = ['new', 'confirmed', 'cancelled', 'completed', 'no_answer'];
  if (!status && note === undefined) {
    return res.status(400).json({ error: "Provide status or note" });
  }
  if (status && !VALID.includes(status)) {
    return res.status(400).json({ error: "Invalid status", allowed: VALID });
  }
  try {
    const rows = await db.select().from(accountsTable).where(eq(accountsTable.hash, hash)).limit(1);
    ...
    const prevStatus = (target['status'] as string) || 'new';
    const updated = existing.map(r => r['id'] === id ? {
      ...r,
      ...(status ? { status } : {}),
      ...(note !== undefined ? { note } : {}),
    } : r);
...
/*
 * GET /api/booking-request-status — публичный, для клиента проверить статус своей записи
 */
router.get("/booking-request-status", async (req, res) => {
  const { targetHash, id } = req.query as { targetHash?: string; id?: string };
  if (!targetHash || !id) return res.status(400).json({ error: "Missing params" });
  try {
    const rows = await db.select().from(accountsTable).where(eq(accountsTable.hash, targetHash)).limit(1);
    if (!rows.length) return res.status(404).json({ error: "Not found" });
    const fullData = (rows[0].data as Record<string, unknown>) || {};
    const requests = Array.isArray(fullData['booking_requests']) ? fullData['booking_requests'] as Record<string,unknown>[] : [];
    const found = requests.find(r => r['id'] === id);
    if (!found) return res.status(404).json({ error: "Not found" });
    return res.json({ status: found['status'], service: found['service'], slot: found['slot'], clientName: found['clientName'], price: found['price'] });
  } catch (e) {
    logger.error({ err: e }, 'accounts error');
    return res.status(500).json({ error: "Server error" });
  }
...
router.post("/booking-request", async (req, res) => {
  const { targetHash, text, clientName, clientPhone, service, slot, price, itemId, channelId, channelName } = req.body as {
    targetHash?: string; text?: string; clientName?: string; clientPhone?: string; service?: string; slot?: string; price?: string; itemId?: string; channelId?: string; channelName?: string;
  };
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      /* Если слот указан — проверяем внутри lock что он ещё доступен.
       * Два типа слотов:
       *   1. pro_freeSlots — свободные слоты без привязки к услуге
       *   2. priceItem slots — слоты конкретной услуги (itemId обязателен)
       * Для priceItem-слотов не удаляем из pro_freeSlots, только проверяем
       * дублирование в booking_requests.
       */
      if (slot) {
        const alreadyBooked = (existing as {slot?:string;status?:string;itemId?:string}[])
          .some(r => {
        ...
        if (alreadyBooked) throw Object.assign(new Error('slot_unavailable'), { status: 409 });
...
/* ══════════════════════════════════════════════════════
   SYNC: channels and groups stored in account data JSONB
   Uses || operator to MERGE, not replace, the account data.
══════════════════════════════════════════════════════ */

/* GET /api/channels-sync — load user's channels array */
router.get("/channels-sync", requireSession, async (req, res) => {
  const hash = (req as any).userHash as string;
  try {
    const rows = await db.select({ data: accountsTable.data }).from(accountsTable).where(eq(accountsTable.hash, hash)).limit(1);
    const data = (rows[0]?.data as Record<string, unknown>) || {};
    return res.json({ channels: Array.isArray(data['sw_channels']) ? data['sw_channels'] : [] });
  } catch (e) {
    logger.error({ err: e }, 'accounts error');
    return res.status(500).json({ error: "Server error" });
  }
...
/* PUT /api/channels-sync — merge channels array into account data */
router.put("/channels-sync", requireSession, async (req, res) => {
  const hash = (req as any).userHash as string;
  ...
  try {
    const patch = JSON.stringify({ sw_channels: channels });
    await db.insert(accountsTable)
      .values({ hash, data: { sw_channels: channels } })
      .onConflictDoUpdate({
        target: accountsTable.hash,
        set: {
          data: sql`${accountsTable.data} || ${patch}::jsonb`,
          updatedAt: new Date(),
        },
      });
    return res.json({ ok: true });
...
/* GET /api/groups-sync — load user's groups array */
router.get("/groups-sync", requireSession, async (req, res) => {
  const hash = (req as any).userHash as string;
  try {
    const rows = await db.select({ data: accountsTable.data }).from(accountsTable).where(eq(accountsTable.hash, hash)).limit(1);
    const data = (rows[0]?.data as Record<string, unknown>) || {};
    return res.json({ groups: Array.isArray(data['sw_groups']) ? data['sw_groups'] : [] });
  } catch (e) {
    logger.error({ err: e }, 'accounts error');
    return res.status(500).json({ error: "Server error" });
  }
...
/* PUT /api/groups-sync — merge groups array into account data */
router.put("/groups-sync", requireSession, async (req, res) => {
  const hash = (req as any).userHash as string;
  const { groups } = req.body as { groups?: unknown };
  if (!Array.isArray(groups)) return res.status(400).json({ error: "groups must be an array" });
  try {
    const patch = JSON.stringify({ sw_groups: groups });
    await db.insert(accountsTable)
      .values({ hash, data: { sw_groups: groups } })
      .onConflictDoUpdate({
        target: accountsTable.hash,
        set: {
          data: sql`${accountsTable.data} || ${patch}::jsonb`,
...
/* ══════════════════════════════════════════════════════
   GET /api/shared-post/:shareId — публичный, для публичной страницы поста
   shareId = base64(ownerHash:postId)
══════════════════════════════════════════════════════ */
router.get("/shared-post/:shareId", async (req, res) => {
  const { shareId } = req.params;
  try {
    const decoded = Buffer.from(shareId, 'base64').toString('utf8');
    const colonIdx = decoded.indexOf(':');
    if (colonIdx === -1) return res.status(400).json({ error: 'Invalid shareId' });
    const ownerHash = decoded.slice(0, colonIdx);
    const postId = decoded.slice(colonIdx + 1);

    const rows = await db.select({ data: accountsTable.data })
      .from(accountsTable).where(eq(accountsTable.hash, ownerHash)).limit(1);
    if (!rows.length) return res.status(404).json({ error: 'Not found' });

    const data = (rows[0].data as Record<string, unknown>) || {};
    const channels = Array.isArray(data['sw_channels']) ? (data['sw_channels'] as Record<string, unknown>[]) : [];

    let foundPost: Record<string, unknown> | null = null;
    let foundChannel: Record<string, unknown> | null = null;
    for (const ch of channels) {
      const posts = Array.isArray(ch['posts']) ? (ch['posts'] as Record<string, unknown>[]) : [];
      const post = posts.find(p => p['id'] === postId);
      if (post) {
        foundPost = post;
        foundChannel = {
          id: ch['id'], name: ch['name'], vibeColor: ch['vibeColor'],
          vibe: ch['vibe'], avatarPhotoUrl: ch['avatarPhotoUrl'],
          handle: ch['handle'],
        };
        break;
      }
    ...
    /* Публичные поля профиля владельца */
    const profileData = data as Record<string, unknown>;
    const ownerProfile = {
      displayName: profileData['pro_displayName'] || profileData['scene_name'] || '',
      avatar: profileData['pro_avatar'] || '',
    };

    return res.json({ post: foundPost, channel: foundChannel, ownerHash, ownerProfile });
  } catch (e) {
    logger.error({ err: e }, 'accounts error');
    return res.status(500).json({ error: 'Server error' });
  }
