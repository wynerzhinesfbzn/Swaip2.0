...
router.get("/mini-apps", requireSession, async (req, res) => {
  ...
  } catch (err) { logger.error({ err }, "miniApps error"); return res.status(500).json({ error: "Server error" }); }
});
...
router.post("/mini-apps", requireSession, async (req, res) => {
  try {
    ...
    return res.json(app);
  } catch (e) { logger.error({ err: e }, "miniApps error"); return res.status(500).json({ error: "Server error" }); }
});
