...
/* ─── Game Proxy — snимает X-Frame-Options/CSP, добавляет COOP/COEP для WASM ─ */
router.get("/game-proxy", async (req, res) => {
  ...
  const proxyBase = `/api/game-proxy?url=`;

  /* Убираем заголовки, которые мог добавить наш же middleware */
  ...
  try {
    ...
    /* CORS для всех ресурсов (нужно для wasm-fetch внутри игр) */
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Cross-Origin-Resource-Policy", "cross-origin");

    if (isHtml) {
      /* COOP/COEP — обязательны для SharedArrayBuffer в Ruffle/WebAssembly */
      res.set("Cross-Origin-Opener-Policy", "same-origin");
      res.set("Cross-Origin-Embedder-Policy", "credentialless");
      /* Разрешающий CSP для игрового контента */
      ...
      /* Бинарные файлы: JS, wasm, ROM, изображения, шрифты */
    ...
    res.status(502).set("Content-Type", "text/html; charset=utf-8").send(`<!DOCTYPE html>
<html><head><meta charset="utf-8"><style>
*{margin:0;padding:0;box-sizing:border-box}
body{background:#0a0a14;color:#fff;font-family:"Montserrat",sans-serif;
     display:flex;align-items:center;justify-content:center;height:100vh;text-align:center;padding:24px}
.w{max-width:300px}.i{font-size:48px;margin-bottom:16px}
h2{font-size:17px;font-weight:900;margin-bottom:10px}
p{font-size:12px;color:rgba(255,255,255,0.5);line-height:1.6}
</style></head><body><div class="w">
  <div class="i">🎮</div>
  <h2>Игра недоступна</h2>
  <p>Игровой сервер не отвечает или временно недоступен.<br>Попробуй другую игру или повтори попытку позже.</p>
</div></body></html>`);
  }
});

/* ─── Browser Proxy (general) ─────────────────────────────────── */
...
p{font-size:13px;color:rgba(255,255,255,0.6);line-height:1.6;margin-bottom:16px}
.hint{padding:12px 16px;background:rgba(124,58,237,0.15);
      border:1px solid rgba(124,58,237,0.3);border-radius:12px;
      font-size:12px;color:rgba(167,139,250,0.9);line-height:1.6}
b{color:#c4b5fd}
</style></head>
<body><div class="wrap">
  <div class="icon">🚫</div>
  <h2>Сайт недоступен</h2>
  <p>Этот сайт заблокировал встроенную загрузку или недоступен из данной сети.</p>
  <div class="hint">
    Вставь ссылку <b>YouTube</b>, <b>Rutube</b> или <b>ВКонтакте</b> — они встраиваются напрямую и работают всегда.
  </div>
</div></body></html>`);
  }
});
