...
router.post("/video-upload/init", async (req: Request, res: Response): Promise<void> => {
  const userHash = await resolveSession(getSessionToken(req));
  if (!userHash) { res.status(401).json({ error: "Unauthorized" }); return; }
  try {
...
router.post("/video-upload/chunk", async (req: Request, res: Response): Promise<void> => {
  const userHash = await resolveSession(getSessionToken(req));
  if (!userHash) { res.status(401).json({ error: "Unauthorized" }); return; }
  try {
...
router.post("/video-upload/finalize", async (req: Request, res: Response): Promise<void> => {
  const userHash = await resolveSession(getSessionToken(req));
  if (!userHash) { res.status(401).json({ error: "Unauthorized" }); return; }
  try {
    const rawBody = Buffer.isBuffer(req.body) ? JSON.parse(req.body.toString("utf8")) : req.body;
...
router.post("/video-upload", async (req: Request, res: Response): Promise<void> => {
  const userHash = await resolveSession(getSessionToken(req));
  if (!userHash) { res.status(401).json({ error: "Unauthorized" }); return; }
  try {
    const buf: Buffer = Buffer.isBuffer(req.body) ? req.body : Buffer.from(req.body || "");
