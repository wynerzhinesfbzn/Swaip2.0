...
function saveInvites(d: Record<string, InviteRecord>): void {
  try {
    ...
    logger.error({ err }, "[channelInvites] saveInvites failed");
  }
}
...
function saveHandles(d: Record<string, HandleRecord>): void {
  try {
    ...
    logger.error({ err }, "[channelInvites] saveHandles failed");
  }
}
...
router.get("/channel-handles/check", async (req, res) => {
  ...
  try {
    ...
    logger.error({ err }, "[channelInvites] DB handle check failed");
  }
