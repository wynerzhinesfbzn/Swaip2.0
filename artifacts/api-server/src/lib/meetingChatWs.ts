...
export function attachMeetingChatWs(): WebSocketServer {
    ...
    ws.on("message", (raw) => {
      try {
        ...
        if (msg.type === "mute_request") {
          ...
          const room = rooms.get(meetingId);
          if (!room) return;
          const payload = JSON.stringify({ type: "mute_request", fromId: participantId });
          for (const client of room) {
            if (client.participantId === targetId && client.ws.readyState === WebSocket.OPEN) {
